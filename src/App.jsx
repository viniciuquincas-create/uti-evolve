...
const SISTEMAS = [
  "NeurolÃ³gico","RespiratÃ³rio","HemodinÃ¢mico",
  "Renal/MetabÃ³lico","Gastrointestinal","HematolÃ³gico/Infeccioso","Pele/Acessos",
];
...
const METAS_SUGESTOES = [
  "Meta de diurese > 0,5 mL/kg/h","Desmame ventilatÃ³rio â reduzir FiO2",
  "Controle glicÃªmico 140-180 mg/dL","MobilizaÃ§Ã£o precoce",
  "ReposiÃ§Ã£o de K+ se < 3,5","Hemoculturas antes de ATB",
  "Ecocardiograma beira-leito","Discutir retirada de DVA",
];

// DiluiÃ§Ãµes padrÃ£o do protocolo da UTI
// concMcgML = mcg de fÃ¡rmaco por mL da soluÃ§Ã£o final
// unidade = unidade da dose resultante exibida ao usuÃ¡rio
...
const DROGAS_PROTOCOLO = {
  noradrenalina: {
    ...
    diluicaoDesc:"4 amp (16 mg) em SG5% 234 mL â 250 mL",
    concMcgML: 64,
    modoCalcDefault:"mcg_kg_min",
    modoCalcOpcoes:["mcg_kg_min"],
    max:3, unidadeLabel:"mcg/kg/min",
    doseInfo:"0,01 â 3 mcg/kg/min\nDose vasoconstritora: 0,1â0,3 mcg/kg/min\nAlerta: > 1 mcg/kg/min = dose muito alta",
  },
  dobutamina: {
    label:"Dobutamina", grupo:"vasoativa",
    diluicaoDesc:"80 mL (250 mg) em SG5% 170 mL â 250 mL",
    concMcgML: 1000,
    modoCalcDefault:"mcg_kg_min",
    modoCalcOpcoes:["mcg_kg_min"],
    max:20, unidadeLabel:"mcg/kg/min",
    doseInfo:"2 â 20 mcg/kg/min\nEfeito inotrÃ³pico: 2â10 mcg/kg/min\nEfeito cronotrÃ³pico relevante: > 10 mcg/kg/min",
  },
  vasopressina: {
    label:"Vasopressina", grupo:"vasoativa",
    diluicaoDesc:"2 mL (20 UI) em SG5% 98 mL â 100 mL",
    concMcgML: null, concUIML: 0.2,
    modoCalcDefault:"ui_min",
    modoCalcOpcoes:["ui_min"],
    max:0.04, unidadeLabel:"UI/min",
    doseInfo:"0,01 â 0,04 UI/min\nUsado como adjuvante fixo ao lado da noradrenalina\nNÃ£o titular â dose fixa de 0,03â0,04 UI/min",
  },
  nitroglicerina: {
    label:"Nitroglicerina", grupo:"vasoativa",
    diluicaoDesc:"10 mL (50 mg) em SG5% 90 mL â 100 mL",
    concMcgML: 500,
    modoCalcDefault:"mcg_min",
    modoCalcOpcoes:["mcg_min","mcg_kg_min"],
    max:400, unidadeLabel:"mcg/min",
    doseInfo:"5 â 400 mcg/min\nCrise hipertensiva/angina: 5â200 mcg/min\nEfeito predominantemente venoso: doses baixas\nEfeito arterial: doses altas (> 200 mcg/min)",
  },
  nitroprussiato: {
    label:"Nitroprussiato", grupo:"vasoativa",
    diluicaoDesc:"2 mL (50 mg) em SG5% 248 mL â 250 mL",
    concMcgML: 200,
    modoCalcDefault:"mcg_kg_min",
    modoCalcOpcoes:["mcg_kg_min"],
    max:10, unidadeLabel:"mcg/kg/min",
    doseInfo:"0,3 â 10 mcg/kg/min\nInÃ­cio com 0,3â0,5 mcg/kg/min\nAlerta de toxicidade por cianeto: > 4 mcg/kg/min por > 3 dias",
  },
  propofol: {
    label:"Propofol", grupo:"sedacao",
    diluicaoDesc:"10 mg/mL â 100 mL puro (sem diluiÃ§Ã£o)",
    concMcgML: 10000,
    modoCalcDefault:"mg_kg_h",
    modoCalcOpcoes:["mg_kg_h","mcg_kg_min","mg_h"],
    max:4, unidadeLabel:"mg/kg/h",
    doseInfo:"5 â 50 mcg/kg/min  (= 0,3 â 3 mg/kg/h)\nSedaÃ§Ã£o leve: 5â10 mcg/kg/min\nSedaÃ§Ã£o profunda: 25â50 mcg/kg/min\nAlerta PRIS: > 4 mg/kg/h por > 48h",
  },
  midazolam: {
    label:"Midazolam", grupo:"sedacao",
    diluicaoDesc:"20 mL (100 mg) em SG5% 80 mL â 100 mL",
    concMcgML: 1000,
    modoCalcDefault:"mcg_kg_h",
    modoCalcOpcoes:["mcg_kg_h","mg_kg_h"],
    max:150, unidadeLabel:"mcg/kg/h",
    doseInfo:"0,01 â 0,2 mg/kg/h  (= 10 â 200 mcg/kg/h)\nSedaÃ§Ã£o leve: 0,02â0,05 mg/kg/h\nSedaÃ§Ã£o moderada: 0,05â0,15 mg/kg/h\nEvitar uso prolongado: acÃºmulo e sÃ­ndrome de abstinÃªncia",
  },
  fentanil: {
    label:"Fentanil", grupo:"analgesia",
    diluicaoDesc:"20 mL (1000 mcg) em SF0,9% 80 mL â 100 mL",
    concMcgML: 10,
    modoCalcDefault:"mcg_kg_h",
    modoCalcOpcoes:["mcg_kg_h","mcg_kg_min"],
    max:5, unidadeLabel:"mcg/kg/h",
    doseInfo:"20 â 50 mcg/hora  (dose sem ajuste por peso)\nCom ajuste: 0,5 â 3 mcg/kg/h\nAnalgesia em VM: 25â100 mcg/h\nAtingir score de dor â¤ 3 (BPS ou CPOT)",
  },
  precedex: {
    label:"Precedex (Dex)", grupo:"sedacao",
    diluicaoDesc:"4 mL (200 mcg) em SF0,9% 96 mL â 100 mL",
    concMcgML: 2,
    modoCalcDefault:"mcg_kg_h",
    modoCalcOpcoes:["mcg_kg_h"],
    max:0.7, unidadeLabel:"mcg/kg/h",
    doseInfo:"0,2 â 1,5 mcg/kg/h\nSem ventilaÃ§Ã£o mecÃ¢nica: 0,2â0,7 mcg/kg/h\nCom VM: pode usar atÃ© 1,5 mcg/kg/h\nVantagem: manutenÃ§Ã£o da cooperaÃ§Ã£o (sedaÃ§Ã£o colaborativa)",
  },
  cetamina: {
    label:"Cetamina (S+)", grupo:"analgesia",
    diluicaoDesc:"10 mL escetamina (500 mg) em SG5% 90 mL â 100 mL Â· 5 mg/mL",
    concMcgML: 5000,
    modoCalcDefault:"mg_kg_h",
    modoCalcOpcoes:["mg_kg_h","mcg_kg_min"],
    max:1.0, unidadeLabel:"mg/kg/h",
    doseInfo:"0,06 â 0,3 mg/kg/h  (analgesia adjuvante)\nAnalgesia subanestÃ©sica: 0,1â0,3 mg/kg/h\nDose alta (sedaÃ§Ã£o): 0,5â1,0 mg/kg/h\nVantagem: broncodilataÃ§Ã£o, preserva drive respiratÃ³rio",
  },
};

// Modos de cÃ¡lculo disponÃ­veis com labels
const MODOS_CALC = {
...
const PROC_SUGESTOES = [
  ...
  "Drenagem de tÃ³rax","AmputaÃ§Ã£o","Fasciotomia","Embolectomia",
  "Bypass coronariano","Troca valvar","ECMO","DiÃ¡lise â inÃ­cio",
  "Acesso venoso central","Cateter de artÃ©ria pulmonar",
];

// ââ ProcedimentosPanel ââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function ProcedimentosPanel({ procedimentos=[], onChange }) {
  ...
  return (
    <div>
      <SecTitle>PROCEDIMENTOS CIRÃRGICOS / INVASIVOS</SecTitle>
        ...
        return (
          <div key={p.id} style={{display:"flex",alignItems:"stretch",gap:4,marginBottom:8}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:2,justifyContent:"center"}}>
              ...
              }} style={{background:"none",border:"none",color:pidx===0?"#1e293b":"#64748b",cursor:pidx===0?"default":"pointer",fontSize:11,padding:"2px 4px"}}>â²</button>
              <button onClick={()=>{
                if(pidx===procedimentos.length-1) return;
                const n=[...procedimentos];[n[pidx],n[pidx+1]]=[n[pidx+1],n[pidx]];onChange(n);
              }} style={{background:"none",border:"none",color:pidx===procedimentos.length-1?"#1e293b":"#64748b",cursor:pidx===procedimentos.length-1?"default":"pointer",fontSize:11,padding:"2px 4px"}}>â¼</button>
            </div>
            ...
            <div style={{flex:1,paddingLeft:4}}>
              {editing ? (
                <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                  ...
                  <button onClick={()=>setEditId(null)} style={{padding:"5px 10px",borderRadius:6,border:"1px solid #38bdf8",background:"rgba(56,189,248,0.1)",color:"#38bdf8",cursor:"pointer",fontSize:12}}>â Ok</button>
                </div>
              ) : (
                <>
            ...
            {!editing && (
              <div style={{display:"flex",flexDirection:"column",gap:4}}>
                <button onClick={()=>setEditId(p.id)} title="Editar" style={{background:"none",border:"none",color:"#475569",cursor:"pointer",fontSize:13,padding:2}}>âï¸</button>
                <button onClick={()=>removeProc(p.id)} title="Remover" style={{background:"none",border:"none",color:"#475569",cursor:"pointer",fontSize:13,padding:2}}>ðï¸</button>
              </div>
            )}
          </div>
          </div>
      ...
      <div style={{marginTop:12,padding:"14px",background:"rgba(56,189,248,0.04)",border:"1px solid rgba(56,189,248,0.12)",borderRadius:10}}>
        ...
        <button onClick={()=>setShowSug(s=>!s)} style={{background:"none",border:"none",color:"#475569",fontSize:11,cursor:"pointer",padding:0,fontFamily:mono,letterSpacing:0.5}}>
          {showSug?"â² ocultar sugestÃµes":"â¼ sugestÃµes rÃ¡pidas"}
        </button>
        {showSug && (
      ...
      {procedimentos.length > 0 && (
        <div style={{display:"flex",gap:14,marginTop:10,flexWrap:"wrap"}}>
          {[["#f87171","POI / D0"],["#fb923c","PO1â3"],["#fbbf24","PO4â7"],["#34d399","PO8+"]].map(([c,l])=>(
...
// ââ DrogasCalculadora âââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
const GRUPOS = { vasoativa:"Vasoativas", sedacao:"SedaÃ§Ã£o", analgesia:"Analgesia" };
...
function DrogasCalculadora({ peso, onLancarDroga, vazoes={}, onVazaoChange, config={} }) {
  ...
  // Mapeia grupo â campo da evoluÃ§Ã£o
  ...
  return (
    <div>
      <div style={{fontSize:12,color:"#64748b",marginBottom:12}}>
        Informe a <strong style={{color:"#e2e8f0"}}>vazÃ£o da bomba (mL/h)</strong> â o sistema calcula a dose com base na diluiÃ§Ã£o padrÃ£o do protocolo.
      </div>
                  ...
                  <div style={{display:"flex",border:`1px solid ${isSel?"#38bdf8":temVazao?"rgba(251,146,60,0.6)":"rgba(255,255,255,0.1)"}`,borderRadius:20,background:isSel?"rgba(56,189,248,0.14)":temVazao?"rgba(251,146,60,0.1)":"rgba(255,255,255,0.02)",overflow:"hidden"}}>
                    ...
                    {d.doseInfo && (
                      <button onClick={e=>{e.stopPropagation();setInfoAberta(infoOpen?null:key);}}
                        ...
                        title="Ver doses de referÃªncia">â</button>
                    )}
                  </div>
                  {infoOpen && d.doseInfo && (
                    <div style={{position:"absolute",top:"calc(100% + 6px)",left:0,zIndex:50,minWidth:260,maxWidth:320,padding:"12px 14px",background:"#102010",border:"1px solid rgba(167,139,250,0.35)",borderRadius:10,boxShadow:"0 8px 24px rgba(0,0,0,0.5)"}}>
                      <div style={{fontSize:10,color:"#c4b5fd",fontFamily:mono,letterSpacing:1,marginBottom:6}}>ð DOSES DE REFERÃNCIA â {d.label.toUpperCase()}</div>
                      {d.doseInfo.split("\n").map((l,i)=>(
                        <div key={i} style={{fontSize:12,color:i===0?"#e2e8f0":"#94a3b8",lineHeight:1.6,fontWeight:i===0?700:400}}>{l}</div>
                      ))}
                      <button onClick={()=>setInfoAberta(null)} style={{marginTop:8,background:"none",border:"none",color:"#475569",fontSize:10,cursor:"pointer",padding:0}}>â Fechar</button>
                    </div>
                  )}
                </div>
              );
      ...
      <div style={{marginTop:14,padding:"14px 16px",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12,flexWrap:"wrap",gap:8}}>
          <div>
            ...
            {conf.concMcgML && <div style={{fontSize:11,color:concCustom?"#f59e0b":"#38bdf8",marginTop:1,fontFamily:mono}}>
              {concCustom ? `â ${concCustom} mcg/mL (personalizado)` : `= ${conf.concMcgML} mcg/mL`}
            </div>}
          ...
          <button onClick={()=>setEditandoConc(e=>!e)} style={{padding:"4px 10px",borderRadius:6,border:"1px solid rgba(255,255,255,0.1)",background:"rgba(255,255,255,0.04)",color:"#64748b",fontSize:11,cursor:"pointer",whiteSpace:"nowrap"}}>
            {editandoConc?"â Fechar":"âï¸ DiluiÃ§Ã£o personalizada"}
          </button>
        </div>

        {/* Seletor de modo de cÃ¡lculo (se mais de 1 opÃ§Ã£o) */}
        ...
        {editandoConc && (
          <div style={{marginBottom:14,padding:"10px 12px",background:"rgba(245,158,11,0.07)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:8}}>
            <div style={{fontSize:10,color:"#f59e0b",fontFamily:mono,letterSpacing:1,marginBottom:8}}>CONCENTRAÃÃO PERSONALIZADA (mcg/mL)</div>
            <div style={{display:"flex",gap:8,alignItems:"flex-end",flexWrap:"wrap"}}>
              <Field label="CONCENTRAÃÃO" value={concCustom} onChange={setConcCustom} type="number" placeholder={String(conf.concMcgML||"")} suffix="mcg/mL"/>
            ...
            <div style={{fontSize:11,color:"#64748b",marginTop:6}}>PadrÃ£o do protocolo: {conf.concMcgML} mcg/mL</div>
          </div>
        )}

        <div style={{display:"flex",gap:10,alignItems:"flex-end",flexWrap:"wrap"}}>
          <Field label="VAZÃO DA BOMBA (mL/h)" value={mlh} onChange={setMlh} type="number" placeholder="5.0" suffix="mL/h"/>
          <div style={{flex:1,minWidth:150}}>
            ...
            <div style={{padding:"9px 14px",borderRadius:8,textAlign:"center",background:resBg,border:`1px solid ${resBorder}`,fontSize:16,fontWeight:700,color:resCor,minHeight:38,display:"flex",alignItems:"center",justifyContent:"center"}}>
              {resultado ? `${fmtDose(resultado.dose)} ${resultado.label}` : "â"}
            </div>
          </div>
        </div>
        ...
        {acimaDose && (
          <div style={{marginTop:8,padding:"6px 10px",background:"rgba(248,113,113,0.08)",border:"1px solid rgba(248,113,113,0.25)",borderRadius:6,fontSize:12,color:"#f87171"}}>
            â ï¸ Acima do mÃ¡ximo recomendado: {conf.max} {conf.unidadeLabel}
          </div>
        )}
        {resultado && !acimaDose && conf.max && (
          <div style={{marginTop:6,fontSize:11,color:"#475569"}}>
            MÃ¡x. recomendado: {conf.max} {conf.unidadeLabel}
          </div>
        )}
        {resultado && onLancarDroga && (
          <button onClick={lancarNaEvolucao} style={{
            ...
            {lancado ? "â LanÃ§ado na evoluÃ§Ã£o!" : `ð LanÃ§ar na evoluÃ§Ã£o (${conf.grupo === "vasoativa" ? "== Cv: DVA" : conf.grupo === "sedacao" ? "== N: SedaÃ§Ã£o" : "== N: Analgesia"})`}
          </button>
        )}
      </div>
    </div>
...
// ââ CatÃ¡logo de dietas padrÃ£o âââââââââââââââââââââââââââââââââââââââââââââââââ
const DIETAS_DEFAULT = [
  // ââ Enterais â nomenclatura TASY / Hospital SÃ£o Paulo ââââââââââââââââââââ
  { id:"fresubin_orig",      tipo:"enteral",
    nome:"Dieta Enteral NormocalÃ³rica e Normoproteica S.F",
    comercial:"Fresubin Original",
    kcalML:1.00, ptnML:0.038, choML:0.138, lipML:0.034 },
  { id:"fresubin_12hp",      tipo:"enteral",
    nome:"Dieta Enteral NormocalÃ³rica e Hiperproteica S.F",
    comercial:"Fresubin 1.2 HP Fibre",
    kcalML:1.20, ptnML:0.056, choML:0.144, lipML:0.044 },
  { id:"fresubin_hp_energy", tipo:"enteral",
    nome:"Dieta Enteral HipercalÃ³rica e Hiperproteica s/sacarose S.F",
    comercial:"Fresubin HP Energy",
    kcalML:1.50, ptnML:0.075, choML:0.170, lipML:0.058 },
  { id:"fresubin_2kcal_hp",  tipo:"enteral",
    nome:"Dieta Enteral HipercalÃ³rica e Hiperproteica 2.0 kcal S.F (sem fibras)",
    comercial:"Fresubin 2Kcal HP",
    kcalML:2.00, ptnML:0.100, choML:0.217, lipML:0.088 },
  { id:"fresubin_2kcal_hpf", tipo:"enteral",
    nome:"Dieta Enteral HipercalÃ³rica e Hiperproteica 2.0 kcal S.F (com fibras)",
    comercial:"Fresubin 2Kcal HP Fibre",
    kcalML:2.00, ptnML:0.100, choML:0.200, lipML:0.090 },
  { id:"survimed_opd",       tipo:"enteral",
    nome:"Dieta Enteral Elementar NormocalÃ³rica e Normoproteica S.F",
    comercial:"Survimed OPD",
    kcalML:1.00, ptnML:0.034, choML:0.144, lipML:0.034 },
  { id:"survimed_opd_hn",    tipo:"enteral",
    nome:"Dieta Enteral Elementar HipercalÃ³rica Hiperproteica S.F",
    comercial:"Survimed OPD HN",
    kcalML:1.33, ptnML:0.066, choML:0.175, lipML:0.044 },
  // ââ Parenterais â nomenclatura TASY / Hospital SÃ£o Paulo âââââââââââââââââ
  { id:"olimel_n7",    tipo:"parenteral",
    nome:"NP NORMOPROTEICA tricompartimentada poliaminoÃ¡cidos + glicose + lipÃ­deo - 1000 mL (central)",
    comercial:"Olimel N7 â Baxter",
    kcalML:1.03, ptnML:0.057, choML:0.110, lipML:0.040 },
  { id:"olimel_n9",    tipo:"parenteral",
    nome:"NP HIPERPROTEICA tricompartimentada poliaminoÃ¡cidos + glicose + lipÃ­deo - 1000 mL (central)",
    comercial:"Olimel N9 â Baxter",
    kcalML:1.05, ptnML:0.072, choML:0.100, lipML:0.040 },
  { id:"olig_trat",    tipo:"parenteral",
    nome:"Oligoelementos para nutriÃ§Ã£o parenteral total - 2 mL",
    comercial:"OLIG-TRATÂ®",
    kcalML:0, ptnML:0, choML:0, lipML:0 },
  { id:"cerne12",      tipo:"parenteral",
    nome:"PolivitamÃ­nicos - 5 mL pÃ³ liofilizado",
    comercial:"Cerne 12 â Baxter",
    kcalML:0, ptnML:0, choML:0, lipML:0 },
...
// ââ UtilitÃ¡rios de nutriÃ§Ã£o ââââââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  return (
    <div style={{flex:1,minWidth:130}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",fontSize:11}}>
        <span style={{color:"#e2e8f0",fontWeight:700}}>{recebeu ?? "â"}</span>
        <span style={{color:"#475569"}}>meta {meta ?? "â"}</span>
      </div>
    </div>
  );
}

// ââ DietaPanel ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function DietaPanel({ dados, onChange, config={}, diureseHojeVol="" }) {
  ...
  const TIPOS = [
    {k:"enteral",   label:"ð¥¤ Enteral"},
    {k:"parenteral",label:"ð Parenteral"},
    {k:"oral",      label:"ð½ï¸ Oral"},
    {k:"mista",     label:"ð Mista"},
    {k:"jejum",     label:"â Jejum"},
  ];
    ...
    <div>
      ...
      {dieta.tipo==="jejum" ? (
        <div style={{padding:"12px 14px",background:"rgba(248,113,113,0.07)",border:"1px solid rgba(248,113,113,0.2)",borderRadius:8,fontSize:13,color:"#fca5a5",marginBottom:10}}>
          â Em jejum â registre o motivo nas observaÃ§Ãµes.
        </div>
      ) : dieta.tipo==="oral" ? (
        <div style={{padding:"12px 14px",background:"rgba(56,189,248,0.06)",border:"1px solid rgba(56,189,248,0.2)",borderRadius:8,fontSize:13,color:"#86efac",marginBottom:10}}>
          ð½ï¸ Dieta oral â registre aceitaÃ§Ã£o e consistÃªncia nas observaÃ§Ãµes.
        </div>
      ) : (
        <>
          {/* FÃ³rmula */}
          <div style={{marginBottom:12}}>
            <div style={{fontSize:10,color:"#64748b",fontFamily:mono,letterSpacing:1,marginBottom:5}}>FÃRMULA / DIETA</div>
                ...
                {dietaSel ? (
                  <div>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8,marginTop:3,flexWrap:"wrap"}}>
                      {dietaSel.comercial && <span style={{fontSize:10,color:"#a78bfa",fontFamily:mono}}>â³ {dietaSel.comercial}</span>}
                      {dietaSel.kcalML > 0 && <span style={{fontSize:10,color:"#64748b",fontFamily:mono}}>{dietaSel.kcalML} kcal/mL Â· {(dietaSel.ptnML*100).toFixed(1)} g ptn/100mL</span>}
                    </div>
                  </div>
                ) : "ð Selecionar do catÃ¡logo..."}
              </button>
              {dietaSel && <button onClick={()=>{upd("catalogId","");upd("formula","");}} style={{padding:"6px 10px",borderRadius:6,border:"1px solid rgba(255,255,255,0.1)",background:"rgba(255,255,255,0.03)",color:"#64748b",fontSize:11,cursor:"pointer"}}>â</button>}
            </div>
            {showCatalog && (
              <div style={{marginTop:6,background:"#0c1a10",border:"1px solid rgba(56,189,248,0.2)",borderRadius:10,maxHeight:200,overflowY:"auto",padding:"4px"}}>
                {filtrados.length===0 ? <div style={{padding:"12px",textAlign:"center",color:"#475569",fontSize:12}}>Adicione fÃ³rmulas em âï¸ ConfiguraÃ§Ãµes.</div>
                  : filtrados.map(d=>(
                    <button key={d.id} onClick={()=>{
                      ...
                      <div style={{fontWeight:600,lineHeight:1.4,marginBottom:2}}>
                        {d.nome}
                        {d.id.startsWith("custom_")&&<span style={{fontSize:9,color:"#c4b5fd",marginLeft:4}}> â</span>}
                      </div>
                      <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                        {d.comercial && <span style={{fontSize:10,color:"#a78bfa",fontFamily:mono}}>â³ {d.comercial}</span>}
                        {d.kcalML > 0 && <span style={{fontSize:10,color:"#64748b",fontFamily:mono}}>{d.kcalML} kcal/mL Â· {(d.ptnML*100).toFixed(1)} g ptn</span>}
                      </div>
                    </button>
                  ))
                }
          ...
          {/* VazÃ£o atual (exame fÃ­sico) */}
          <div style={{marginBottom:14}}>
            <div style={{fontSize:10,color:"#64748b",fontFamily:mono,letterSpacing:1,marginBottom:5}}>VAZÃO ATUAL (para o exame fÃ­sico)</div>
            <div style={{display:"flex",gap:10,alignItems:"center"}}>
              ...
              {dieta.vazao && <div style={{fontSize:12,color:"#64748b"}}>= {(parseFloat(dieta.vazao)*20).toFixed(0)} mL em 20h Â· {(parseFloat(dieta.vazao)*24).toFixed(0)} mL/24h</div>}
            </div>
            <div style={{fontSize:10,color:"#475569",marginTop:4}}>â¹ï¸ O volume real que entrou Ã© registrado nos <strong style={{color:"#38bdf8"}}>Controles 24h</strong> â Vol. Dieta.</div>
          </div>
          ...
          <div style={{padding:"12px 14px",background:"rgba(167,139,250,0.06)",border:"1px solid rgba(167,139,250,0.2)",borderRadius:10,marginBottom:14}}>
            <div style={{fontSize:10,color:"#c4b5fd",fontFamily:mono,letterSpacing:1,marginBottom:10}}>ð¯ METAS NUTRICIONAIS</div>
            ...
            {metaAbs && (
              <div style={{marginTop:10,padding:"8px 12px",background:"rgba(167,139,250,0.08)",borderRadius:7,fontSize:11,color:"#c4b5fd"}}>
                ð¯ Meta: <strong>{metaAbs.kcal ? `${metaAbs.kcal} kcal` : "â"}</strong> Â· <strong>{metaAbs.ptn ? `${metaAbs.ptn} g ptn` : "â"}</strong> /dia
              </div>
            )}
          </div>
          ...
          {dietaSel && metaAbs && (
            <div style={{padding:"12px 14px",background:"rgba(56,189,248,0.04)",border:"1px solid rgba(56,189,248,0.12)",borderRadius:10,marginBottom:14}}>
              <div style={{fontSize:10,color:"#38bdf8",fontFamily:mono,letterSpacing:1,marginBottom:10}}>
                ð ATINGIMENTO HOJE
                {!volHoje&&<span style={{color:"#475569",fontWeight:400,marginLeft:8}}>â registre o Vol. Dieta nos Controles 24h</span>}
              </div>
          ...
          {!dietaSel && (
            <div style={{fontSize:11,color:"#64748b",marginBottom:12,padding:"8px 12px",background:"rgba(255,255,255,0.02)",borderRadius:6}}>
              Selecione uma fÃ³rmula para calcular. Cadastre novas em âï¸ <strong style={{color:"#38bdf8"}}>ConfiguraÃ§Ãµes</strong>.
            </div>
          )}
        </>
      )}

      <Field label="OBSERVAÃÃES" value={dieta.obs} onChange={v=>upd("obs",v)} placeholder="Tolerando, vÃ´mitos, resÃ­duo gÃ¡strico, data de introduÃ§Ã£oâ¦"/>
    </div>
  );
}
...
const DISP_SINGULAR = [
  { key:"tot",   label:"Tubo Orotraqueal (TOT)", icone:"ð«", siteDefault:"",         alertaDias:99 },
  { key:"tqt",   label:"Traqueostomia (TQT)",    icone:"ð«", siteDefault:"",         alertaDias:99 },
  { key:"svd",   label:"Sonda Vesical de Demora",icone:"ð§", siteDefault:"",         alertaDias:14 },
  { key:"pai",   label:"Cateter Arterial (PAI)", icone:"ð", siteDefault:"Radial D", alertaDias:7  },
  { key:"sng",   label:"Sonda Naso/Nasoenteral", icone:"ð§", siteDefault:"",         alertaDias:21 },
];

// Dispositivos mÃºltiplos (podem ter N instÃ¢ncias)
const DISP_MULTIPLO = [
  { key:"cvc",    label:"Cateter Venoso Central", icone:"ð©¸", siteDefault:"Jugular interna D", alertaDias:7  },
  { key:"dialise",label:"Cateter de DiÃ¡lise",     icone:"ð´", siteDefault:"Jugular interna D", alertaDias:14 },
  { key:"dreno",  label:"Dreno",                  icone:"ð¥", siteDefault:"",                  alertaDias:21 },
];
    ...
    <div style={{
      ...
      <div style={{padding:"0 14px 12px",borderTop:"1px solid rgba(255,255,255,0.04)",paddingTop:10,display:"flex",gap:8,flexWrap:"wrap"}}>
        <div style={{flex:1,minWidth:120}}>
          <div style={{fontSize:10,color:"#64748b",fontFamily:mono,letterSpacing:1,marginBottom:4}}>DATA INSERÃÃO</div>
        ...
        <div style={{flex:1,minWidth:130}}>
          <div style={{fontSize:10,color:"#64748b",fontFamily:mono,letterSpacing:1,marginBottom:4}}>SÃTIO / LOCALIZAÃÃO</div>
          <input value={disp.site||""} onChange={e=>onUpdate("site",e.target.value)} placeholder="Ex: Femoral E / TÃ³rax D / PeritÃ´nio"
            style={{width:"100%",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"7px 10px",color:"#e2e8f0",fontSize:12,fontFamily:"inherit"}}/>
        </div>
        <div style={{flex:2,minWidth:160}}>
          <div style={{fontSize:10,color:"#64748b",fontFamily:mono,letterSpacing:1,marginBottom:4}}>OBSERVAÃÃES</div>
          <input value={disp.obs||""} onChange={e=>onUpdate("obs",e.target.value)} placeholder="Curativo ok, sem sinais de infecÃ§Ã£oâ¦"
            style={{width:"100%",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"7px 10px",color:"#e2e8f0",fontSize:12,fontFamily:"inherit"}}/>
        </div>
      </div>
    </div>
...
function DispositivosPanel({ dispositivos={}, onChange, alertas={} }) {
  ...
  // MÃºltiplo: dispositivos[key] = [ { id, data, site, obs }, ... ]
  ...
  // Quais singulares ainda nÃ£o foram inseridos
  const singularesDisponiveis = DISP_SINGULAR.filter(d => !isSingularAtivo(d.key));
  // MÃºltiplos sempre disponÃ­veis para adicionar mais
  const temAlgumAtivo =
    ...
    <div>
      ...
      <div style={{position:"relative"}}>
        <button onClick={()=>setShowPicker(v=>!v)} style={{
          ...
          <span style={{fontSize:16}}>{showPicker?"â":"+"}</span>
        ...
        {showPicker && (
          <div style={{marginTop:8,padding:"8px",background:"#0c1a10",border:"1px solid rgba(56,189,248,0.2)",borderRadius:12,display:"flex",flexDirection:"column",gap:4}}>
            {/* MÃºltiplos sempre disponÃ­veis */}
            {DISP_MULTIPLO.map(({key,label,icone,siteDefault})=>(
              <button key={key} onClick={()=>inserirMultiplo(key,siteDefault)} style={{
                ...
                <div>
                  ...
                  <div style={{fontSize:10,color:"#38bdf8",fontFamily:mono,marginTop:1}}>pode adicionar mÃºltiplos</div>
                </div>
              </button>
            ))}
            ...
            {singularesDisponiveis.length>0 && (
              <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",margin:"4px 0",paddingTop:4}}>
                <div style={{fontSize:9,color:"#334155",fontFamily:mono,letterSpacing:2,paddingLeft:14,paddingBottom:4}}>DISPOSITIVO ÃNICO</div>
                {singularesDisponiveis.map(({key,label,icone,siteDefault})=>(
                  <button key={key} onClick={()=>inserirSingular(key,siteDefault)} style={{
                    ...
                    <div>
                      <div style={{fontSize:13,fontWeight:600,color:"#cbd5e1"}}>{label}</div>
                      {siteDefault&&<div style={{fontSize:11,color:"#475569",marginTop:1}}>SÃ­tio padrÃ£o: {siteDefault}</div>}
                    </div>
                  </button>
                ))}
              </div>
      ...
      {temAlgumAtivo && (
        <div style={{marginTop:10,fontSize:11,color:"#475569",display:"flex",gap:12,flexWrap:"wrap"}}>
          <span>ðµ Em uso</span>
          <span style={{color:"#f87171"}}>ð´ CVC/PAI &gt;7d Â· SVD &gt;14d Â· DiÃ¡lise &gt;14d â revisar</span>
        </div>
      )}
    </div>
  );
...
// ââ PacientePanel âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function PacientePanel({ dados, onChange, config={}, onLancarDroga, onConfigChange, diureseHoje="", tabelaHoje={} }) {
  ...
  // Diurese: usa o valor dos Controles 24h (c24_diur) com perÃ­odo fixo de 24h
    ...
    <div>
      ...
      {(dias!==null||pp||dados.peso||idade!==null) && <>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          ...
          {vc8          && <Pill label="VC 8 mL/kg"   value={vc8}          unit="mL (mÃ¡x ARDSNet)"color="#34d399"/>}
        </div>
        {pp && (
          <div style={{ marginTop:10, padding:"10px 14px", background:"rgba(251,146,60,0.07)", border:"1px solid rgba(251,146,60,0.25)", borderRadius:8, fontSize:12, color:"#fdba74" }}>
            ð¡ <strong>Peso predito (ARDSNet):</strong> {dados.sexo==="M"?"â":"â"} {dados.altura} cm â {pp} kg.
            Use <strong>{vc6} mL</strong> como ponto de partida para VM protetora (6 mL/kg PP) e nÃ£o ultrapasse <strong>{vc8} mL</strong> (8 mL/kg PP) no SDRA.
          </div>
        )}
      </>}

      {dados.peso && <>
        <SecTitle>CALCULADORA DE DROGAS â VAZÃO â DOSE</SecTitle>
      ...
      <SecTitle>HISTÃRICO CLÃNICO</SecTitle>
      <div style={{marginBottom:10}}>
        <div style={{fontSize:10,color:"#64748b",fontFamily:mono,letterSpacing:1,marginBottom:4}}>DOENÃAS PRÃVIAS / COMORBIDADES</div>
        <textarea value={dados.doencasPrevias||""} onChange={e=>onChange({...dados,doencasPrevias:e.target.value})}
          placeholder={"HAS Â· DM2 Â· ICC (FEVE 35%) Â· DRC estÃ¡gio 3 Â· DPOC Â· FA crÃ´nica..."}
          rows={3}
      ...
      <div style={{marginBottom:10}}>
        <div style={{fontSize:10,color:"#64748b",fontFamily:mono,letterSpacing:1,marginBottom:4}}>MEDICAÃÃES DE USO CONTÃNUO (domiciliar)</div>
...
// ââ UploadAnalyzer ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  const handleFile = useCallback(async (file) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        if (data.raw) throw new Error("Resposta invÃ¡lida da IA");
        setDraft(data);
        setRev(true);
  ...
  return (
    <div>
      <div onDrop={e=>{e.preventDefault();handleFile(e.dataTransfer.files[0]);}} onDragOver={e=>e.preventDefault()} onClick={()=>fileRef.current?.click()}
        ...
        <div style={{color:"#64748b",fontSize:12,marginTop:6}}>ou arraste Â· ou clique para selecionar arquivo</div>
      ...
      {loading && <div style={{textAlign:"center",color:"#38bdf8",padding:16,fontSize:14}}>â³ Analisando imagem com IAâ¦</div>}
        ...
        <div>
          ...
          {draft.dataColeta && (
            <div style={{background:"rgba(56,189,248,0.08)",border:"1px solid rgba(56,189,248,0.25)",borderRadius:8,padding:"8px 14px",marginBottom:12,fontSize:12,color:"#4ade80",display:"flex",alignItems:"center",gap:8}}>
              ð <strong>Data detectada:</strong> {(() => {
                const [datePart] = draft.dataColeta.split('T');
                const [y,m,d] = datePart.split('-');
                return `${d}/${m}/${y}`;
              })()} â os valores serÃ£o lanÃ§ados nesta coluna da tabela
            </div>
          )}

          {/* ââ Controles 24h extraÃ­dos ââ */}
          {draft.controles && Object.values(draft.controles).some(v=>v) && (
            <div style={{marginBottom:16,padding:"12px 14px",background:"rgba(52,211,153,0.06)",border:"1px solid rgba(52,211,153,0.2)",borderRadius:10}}>
              <div style={{fontSize:11,color:"#34d399",fontFamily:mono,letterSpacing:1,marginBottom:10}}>ð CONTROLES 24H DETECTADOS â edite se necessÃ¡rio</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:8}}>
                {[
                  {key:"c24_temp",   label:"T Â°C  (mÃ­n / mÃ¡x)"},
                  {key:"c24_fc",     label:"FC bpm  (mÃ­n / mÃ¡x)"},
                  {key:"c24_fr",     label:"FR irpm  (mÃ­n / mÃ¡x)"},
                  {key:"c24_sat",    label:"SpO2 %  (mÃ­n / mÃ¡x)"},
                  {key:"c24_pam",    label:"PAM mmHg  (mÃ­n / mÃ¡x)"},
                  {key:"c24_pas",    label:"PAS/PAD  (mÃ­n-mÃ¡x / mÃ­n-mÃ¡x)"},
                  {key:"c24_dextro", label:"Glic cap  (mÃ­n / mÃ¡x)"},
                  ...
                  {key:"c24_sng",    label:"ResÃ­duo SNG mL  (total)"},
                ].map(({key,label})=>(
                  <div key={key}>
          ...
          <div style={{fontSize:12,color:"#94a3b8",marginBottom:8,fontFamily:mono}}>DADOS CLÃNICOS (sistemas) â edite se necessÃ¡rio</div>
          {SISTEMAS.map(s=>(
          ...
          {/* Exames extras nÃ£o categorizados */}
          {(draft.extras||[]).length > 0 && (
            <div style={{marginTop:4,marginBottom:12,padding:"12px 14px",background:"rgba(245,158,11,0.07)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:10}}>
              <div style={{fontSize:11,color:"#f59e0b",fontFamily:mono,letterSpacing:1,marginBottom:10}}>â ï¸ EXAMES NÃO CATEGORIZADOS â selecione onde lanÃ§ar</div>
              {(draft.extras||[]).map((ex,i)=>(
                <div key={i} style={{display:"flex",gap:8,alignItems:"center",marginBottom:8,flexWrap:"wrap"}}>
                  ...
                  <select value={ex.categoria||ex.sugestao||""} onChange={e=>setDraft(d=>({...d,extras:d.extras.map((x,j)=>j===i?{...x,categoria:e.target.value}:x)}))}
                    ...
                    <option value="" style={{background:"#111f14",color:"#94a3b8"}}>â Ignorar â</option>
          ...
          <button onClick={()=>{onResult(draft);setRev(false);}}
            ...
            ð Confirmar e adicionar Ã  Tabela ClÃ­nica
          </button>
        </div>
      )}
...
// ââ EvolucaoEditor ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// ââ Helpers de evoluÃ§Ã£o âââââââââââââââââââââââââââââââââââââââââââââââââââââââ
const v = (s) => s?.trim() || "";

function TA({ fieldRef, defaultValue, sugestao, placeholder, rows=2, isAntigo=false, fieldName, onBlurSave }) {
  const [showSug, setShowSug] = useState(false);
  const cleanPlaceholder = placeholder && !sugestao ? placeholder : ""; // sÃ³ usa placeholder se nÃ£o tem sugestÃ£o separada
  return (
    <div style={{position:"relative"}}>
      ...
      {showSug && (sugestao||placeholder) && (
        <div style={{position:"absolute",top:"calc(100% + 4px)",left:0,right:0,zIndex:20,background:"#102010",border:"1px solid rgba(56,189,248,0.3)",borderRadius:8,padding:"8px 10px",boxShadow:"0 6px 20px rgba(0,0,0,0.5)"}}>
          <div style={{fontSize:9,color:"#38bdf8",fontFamily:mono,letterSpacing:1,marginBottom:4}}>SUGESTÃO</div>
          ...
          <button onClick={()=>setShowSug(false)} style={{marginTop:6,marginLeft:4,background:"none",border:"none",color:"#475569",fontSize:10,cursor:"pointer"}}>â</button>
        </div>
      )}
      {isAntigo && (
...
// Bloco de sistema com preview corrido + botÃ£o copiar individual
    ...
    <div style={{marginBottom:10,border:`1px solid ${open?"rgba(255,255,255,0.09)":"rgba(255,255,255,0.05)"}`,borderRadius:10,overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"center",background:"rgba(255,255,255,0.03)"}}>
        <button onClick={()=>setOpen(o=>!o)} style={{
          ...
          <span style={{marginLeft:"auto",color:"#475569",fontSize:11}}>{open?"â²":"â¼"}</span>
        </button>
        {/* BotÃ£o copiar do bloco */}
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,0.05)"}}>
          ...
          {preview?.trim() && (
            <div style={{
              ...
              <div style={{fontSize:9,color:"#334155",fontFamily:mono,letterSpacing:1.5,marginBottom:6}}>PRÃ-VISUALIZAÃÃO â texto que serÃ¡ colado no Tasy</div>
...
const DISP_CONFIG_ITEMS = [
  { key:"alertaCVC",    label:"Cateter Venoso Central",    icone:"ð©¸" },
  { key:"alertaPAI",    label:"Cateter Arterial (PAI)",    icone:"ð" },
  { key:"alertaSVD",    label:"Sonda Vesical de Demora",   icone:"ð§" },
  { key:"alertaDialise",label:"Cateter de DiÃ¡lise",        icone:"ð´" },
  { key:"alertaTOT",    label:"Tubo Orotraqueal (TOT)",    icone:"ð«" },
  { key:"alertaTQT",    label:"Traqueostomia (TQT)",       icone:"ð«" },
  { key:"alertaSNG",    label:"Sonda Naso/Nasoenteral",    icone:"ð§" },
  { key:"alertaDreno",  label:"Dreno",                     icone:"ð¥" },
];
...
function ConfigPanel({ config, onChange, onVoltar }) {
  ...
  return (
    <div style={{width:"100%",maxWidth:"min(100%, 1180px)"}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
        <button onClick={onVoltar} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,color:"#64748b",cursor:"pointer",fontSize:12,padding:"6px 12px"}}>â Voltar</button>
        <div>
          <div style={{fontSize:15,fontWeight:700}}>âï¸ ConfiguraÃ§Ãµes</div>
          <div style={{fontSize:12,color:"#64748b"}}>Dispositivos e catÃ¡logo de dietas</div>
        </div>
      </div>

      {/* Chave Gemini para ImpressÃ£o IA */}
      <div style={{background:"rgba(167,139,250,0.04)",border:"1px solid rgba(167,139,250,0.2)",borderRadius:12,overflow:"hidden",marginBottom:20}}>
        <div style={{padding:"12px 16px",borderBottom:"1px solid rgba(167,139,250,0.12)"}}>
          <div style={{fontSize:11,color:"#c084fc",fontFamily:mono,letterSpacing:2}}>â¨ IA â CHAVE GEMINI</div>
          <div style={{fontSize:11,color:"#64748b",marginTop:2}}>NecessÃ¡ria para gerar a ImpressÃ£o ClÃ­nica com IA na aba EvoluÃ§Ã£o</div>
        </div>
        <div style={{padding:"12px 16px"}}>
          <div style={{fontSize:10,color:"#64748b",fontFamily:mono,marginBottom:5}}>API KEY â obtenha em <a href="https://aistudio.google.com/apikey" target="_blank" rel="noreferrer" style={{color:"#c084fc"}}>aistudio.google.com/apikey</a></div>
          <input
            type="password"
          ...
          {config.geminiKey && <div style={{marginTop:6,fontSize:11,color:"#34d399"}}>â Chave salva â â¨ Gerar com IA estÃ¡ ativo</div>}
          {!config.geminiKey && <div style={{marginTop:6,fontSize:11,color:"#f59e0b"}}>â  Sem chave â o botÃ£o â¨ Gerar com IA nÃ£o funcionarÃ¡</div>}
        </div>
      </div>
      ...
      <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:12,overflow:"hidden",marginBottom:20}}>
        <div style={{padding:"12px 16px",borderBottom:"1px solid rgba(255,255,255,0.06)",background:"rgba(255,255,255,0.02)"}}>
          ...
          <div style={{fontSize:11,color:"#64748b",marginTop:2}}>O dispositivo ficarÃ¡ vermelho â ï¸ apÃ³s este nÃºmero de dias</div>
        </div>
        {DISP_CONFIG_ITEMS.map(({key,label,icone})=>(
          <div key={key} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <button onClick={()=>upd(key,Math.max(1,(config[key]||7)-1))} style={{width:28,height:28,borderRadius:6,border:"1px solid rgba(255,255,255,0.1)",background:"rgba(255,255,255,0.04)",color:"#64748b",cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>â</button>
            ...
            {config[key]===99 && <span style={{fontSize:10,color:"#475569",fontFamily:mono}}>â</span>}
          </div>
        ))}
      </div>
      ...
      <div style={{marginBottom:20,padding:"10px 14px",background:"rgba(245,158,11,0.07)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:8,fontSize:12,color:"#fcd34d"}}>
        ð¡ Dica: para dispositivos sem limite de troca (TOT, TQT), deixe em 99 dias â o alerta nÃ£o serÃ¡ disparado.
      </div>
      ...
      <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:12,overflow:"hidden"}}>
        <div style={{padding:"12px 16px",borderBottom:"1px solid rgba(255,255,255,0.06)",background:"rgba(255,255,255,0.02)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontSize:11,color:"#fb923c",fontFamily:mono,letterSpacing:2}}>CATÃLOGO DE DIETAS</div>
            <div style={{fontSize:11,color:"#64748b",marginTop:2}}>FÃ³rmulas disponÃ­veis para seleÃ§Ã£o nos pacientes</div>
          </div>
          <button onClick={()=>setShowAddDieta(s=>!s)} style={{padding:"5px 12px",background:showAddDieta?"rgba(167,139,250,0.15)":"rgba(255,255,255,0.04)",border:`1px solid ${showAddDieta?"#a78bfa":"rgba(255,255,255,0.1)"}`,borderRadius:7,color:showAddDieta?"#c4b5fd":"#64748b",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
            {showAddDieta?"â Fechar":"+ Nova dieta"}
          </button>
        </div>

        {/* FormulÃ¡rio de nova dieta */}
        {showAddDieta && (
          <div style={{padding:"14px 16px",borderBottom:"1px solid rgba(255,255,255,0.06)",background:"rgba(167,139,250,0.04)"}}>
            <div style={{fontSize:10,color:"#c4b5fd",fontFamily:mono,letterSpacing:1,marginBottom:10}}>NOVA DIETA â valores por mL da fÃ³rmula pronta</div>
            <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:8}}>
              <div style={{flex:2,minWidth:160}}>
            ...
            <div style={{fontSize:10,color:"#475569",marginBottom:8}}>* Insira os valores <strong>por mL</strong> da fÃ³rmula. Ex: kcal/mL = 1.5 significa que cada mL tem 1,5 kcal. Ptn/mL = 0.056 significa 5,6 g de proteÃ­na por 100 mL.</div>
            <div style={{display:"flex",gap:8}}>
        ...
        <div style={{maxHeight:320,overflowY:"auto"}}>
          ...
          {(config.dietasCatalogo||[]).map(d=>(
            <div key={d.id} style={{display:"flex",alignItems:"center",padding:"10px 16px",borderBottom:"1px solid rgba(255,255,255,0.04)",background:"rgba(167,139,250,0.04)"}}>
              <span style={{fontSize:10,color:"#c4b5fd",fontFamily:mono,marginRight:8,flexShrink:0}}>â</span>
              <div style={{flex:1}}>
                <div style={{fontSize:12,fontWeight:700,color:"#e2e8f0"}}>{d.nome}</div>
                <div style={{fontSize:10,color:"#64748b",fontFamily:mono}}>{d.tipo} Â· {d.kcalML} kcal/mL Â· {(d.ptnML*100).toFixed(1)} g ptn/100mL</div>
              </div>
              <button onClick={()=>removerDietaCustom(d.id)} style={{background:"none",border:"none",color:"#475569",cursor:"pointer",fontSize:12,padding:"0 4px"}} title="Remover">â</button>
            </div>
          ))}
          {/* PadrÃµes */}
            ...
            <div key={d.id} style={{display:"flex",alignItems:"center",padding:"9px 16px",borderBottom:"1px solid rgba(255,255,255,0.03)"}}>
              <div style={{flex:1}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:2}}>
                  {d.comercial && <span style={{fontSize:10,color:"#a78bfa",fontFamily:mono}}>â³ {d.comercial}</span>}
                  <span style={{fontSize:10,color:"#475569",fontFamily:mono}}>{d.tipo}{d.kcalML>0?` Â· ${d.kcalML} kcal/mL Â· ${(d.ptnML*100).toFixed(1)} g ptn/100mL`:""}</span>
                </div>
              </div>
            </div>
          ))}
...
// ââ TabelaClinica âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
const GRUPOS_LAB = [
  { grupo:"ð©¸ HematolÃ³gico", params:[
    {key:"hb",    label:"Hemoglobina",      unit:"g/dL"},
    {key:"ht",    label:"HematÃ³crito",      unit:"%"},
    {key:"leuco", label:"LeucÃ³citos",       unit:"mil/mmÂ³"},
    {key:"neut",  label:"NeutrÃ³filos",      unit:"%"},
    {key:"bast",  label:"BastÃµes",          unit:"%"},
    {key:"linf",  label:"LinfÃ³citos",       unit:"%"},
    {key:"plaq",  label:"Plaquetas",        unit:"mil/mmÂ³"},
    {key:"rni",   label:"RNI",              unit:""},
    {key:"ttpa",  label:"TTPA",             unit:"s"},
    {key:"fibri", label:"FibrinogÃªnio",     unit:"mg/dL"},
  ]},
  { grupo:"ð« Renal / MetabÃ³lico", params:[
    {key:"cr",    label:"Creatinina",       unit:"mg/dL"},
    {key:"ur",    label:"Ureia",            unit:"mg/dL"},
    {key:"na",    label:"SÃ³dio",            unit:"mEq/L"},
    {key:"k",     label:"PotÃ¡ssio",         unit:"mEq/L"},
    {key:"mg",    label:"MagnÃ©sio",         unit:"mg/dL"},
    {key:"cai",   label:"CÃ¡lcio iÃ´nico",    unit:"mmol/L"},
    {key:"p",     label:"FÃ³sforo",          unit:"mg/dL"},
    {key:"ph",    label:"pH",               unit:""},
    {key:"hco3",  label:"HCO3",             unit:"mEq/L"},
  ]},
  { grupo:"â¤ï¸ Cardiovascular", params:[
  ...
  { grupo:"ð« RespiratÃ³rio", params:[
    {key:"po2",   label:"pO2",              unit:"mmHg"},
    {key:"pco2",  label:"pCO2",             unit:"mmHg"},
  ]},
  { grupo:"ð« TGI / HepÃ¡tico", params:[
...
// Controles 24h â tabela separada com horÃ¡rios
// Drenos, SNG e evacuaÃ§Ãµes sÃ£o OPCIONAIS â adicionados dinamicamente com nome personalizado
const GRUPOS_CONTROLES = [
  { grupo:"ð¡ MonitorizaÃ§Ã£o Geral", params:[
    {key:"c24_temp",  label:"Temperatura (mÃ­n/mÃ¡x)", unit:"Â°C"},
    {key:"c24_fc",    label:"FC (mÃ­n/mÃ¡x)",           unit:"bpm"},
    {key:"c24_fr",    label:"FR (mÃ­n/mÃ¡x)",           unit:"irpm"},
    {key:"c24_pas",   label:"PAS (mÃ­n/mÃ¡x)",          unit:"mmHg"},
    {key:"c24_pad",   label:"PAD (mÃ­n/mÃ¡x)",          unit:"mmHg"},
    {key:"c24_pam",   label:"PAM (mÃ­n/mÃ¡x)",          unit:"mmHg"},
    {key:"c24_sat",   label:"SpO2 (mÃ­n/mÃ¡x)",         unit:"%"},
    {key:"c24_dextro",label:"Dextro (mÃ­n/mÃ¡x)",       unit:"mg/dL"},
  ]},
  { grupo:"ð¥ Ganhos", params:[
    {key:"c24_diet_vol", label:"Vol. Dieta recebida", unit:"mL"},
  ]},
  { grupo:"ð¤ Perdas", params:[
    {key:"c24_diur",  label:"Diurese",                unit:"mL"},
    {key:"c24_hd",    label:"HemodiÃ¡lise / CRRT (UF)",unit:"mL"},
  ]},
  { grupo:"âï¸ BalanÃ§o", params:[
    {key:"c24_bh",    label:"BalanÃ§o HÃ­drico 24h",    unit:"mL"},
    {key:"c24_bh_ac", label:"BalanÃ§o Acumulado",      unit:"mL"},
  ]},
...
// AbreviaÃ§Ãµes para a evoluÃ§Ã£o e formataÃ§Ã£o especial
const ABREV = {
...
// Formata valor: plaquetas e leucÃ³citos em k quando >= 100
const fmtVal = (key, raw) => {
  if (!raw) return raw;
  const n = parseFloat(raw.replace(',','.'));
  if (isNaN(n)) return raw;
  // Plaquetas e leucÃ³citos: mostrar em k (mil)
  if (["plaq","leuco"].includes(key)) {
    if (n >= 100) return `${Math.round(n)}k`;
    // JÃ¡ estÃ¡ em mil (ex: 11.17 = 11170 -> mostra 11.170k)
    if (n < 100) return `${(n).toFixed(n % 1 === 0 ? 0 : 2)}k`;
  }
  // Remove casas decimais desnecessÃ¡rias
  return n % 1 === 0 ? String(Math.round(n)) : raw.replace(',','.');
};

// ââ OptionalDrenosUI âââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function OptionalDrenosUI({ data, onChange, datas, hoje }) {
  const [show, setShow] = useState(false);
  const [nome, setNome] = useState("");
  const SUGESTOES = ["Dreno abdominal", "Dreno torÃ¡cico D", "Dreno torÃ¡cico E", "Dreno pÃ©lvico", "Dreno Jackson-Pratt", "ResÃ­duo SNG", "EvacuaÃ§Ãµes"];
  ...
  return (
    <div style={{marginBottom:8}}>
      <button onClick={()=>setShow(s=>!s)} style={{
        ...
        {show?"â Fechar":"+ Adicionar dreno / SNG / evacuaÃ§Ãµes"}
      </button>
      {show && (
        <div style={{marginTop:6,padding:"12px 14px",background:"rgba(52,211,153,0.05)",border:"1px solid rgba(52,211,153,0.2)",borderRadius:10}}>
          <div style={{fontSize:10,color:"#34d399",fontFamily:mono,letterSpacing:1,marginBottom:8}}>NOME DO ITEM (ex: Dreno abdominal, ResÃ­duo SNG, EvacuaÃ§Ãµes)</div>
...
function TabelaClinica({ leito, data, onChange, onAplicarEvolucao, config={} }) {
  ...
  // Mostra colunas com dados OU marcadas como visÃ­veis, mais hoje sempre
  // Aceita tanto "2026-04-23" quanto "2026-04-23T05:15"
  const comDados = Object.keys(data).filter(d => {
    if (!d.match(/^\d{4}-\d{2}-\d{2}/)) return false; // ignora chaves que nÃ£o sÃ£o datas
    const vals = data[d] || {};
    // mostra se tem qualquer valor, ou se foi marcada como visÃ­vel
    return vals._visivel || Object.entries(vals).some(([k,v]) => k !== '_visivel' && v);
  });
  const datas = Array.from(new Set([...comDados, hoje])).sort();

  // Extrai exames extras dinÃ¢micos (keys comeÃ§ando com _extra_)
  ...
  const adicionarColuna = () => {
    if (!novaData) return;
    // Marca como visÃ­vel mesmo vazia
  ...
  const gerarEvolucao = () => {
    ...
    // Controles 24h: mantÃ©m string bruta inteira (preserva intervalos "36 / 37.2")
    ...
    // Drenos dinÃ¢micos (_dreno_*) â TGI
    ...
    // Controles â campos certos em cada sistema
    ...
    // TGI: glicemia + drenos dinÃ¢micos
    const tgCtrl = [dextroStr, drenosStr].filter(Boolean).join(" Â· ");
    if (tgCtrl) campos.tg24h = tgCtrl;

    onAplicarEvolucao(campos);
  };

  // NavegaÃ§Ã£o por setas entre cÃ©lulas da tabela
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:8}}>
        <div>
          <div style={{fontSize:15,fontWeight:700,color:T.text1}}>Tabela ClÃ­nica</div>
          <div style={{fontSize:12,color:T.text3}}>Registre valores diÃ¡rios Â· depois aplique na evoluÃ§Ã£o</div>
        </div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          <button onClick={()=>setShowAddCol(v=>!v)} style={{padding:"8px 14px",background:T.bgInput,border:`1px solid ${T.border}`,borderRadius:8,color:T.text2,fontWeight:600,fontSize:12,cursor:"pointer"}}>
            {showAddCol?"â Fechar":"ð Adicionar dia"}
          </button>
          {tabela==="labs" && <button onClick={()=>setShowAddExame(v=>!v)} style={{padding:"8px 14px",background:"rgba(167,139,250,0.08)",border:"1px solid rgba(167,139,250,0.25)",borderRadius:8,color:"#c4b5fd",fontWeight:600,fontSize:12,cursor:"pointer"}}>
            {showAddExame?"â Fechar":"ð§ª Novo exame"}
          </button>}
      ...
      <div style={{display:"flex",gap:4,marginBottom:14,background:T.bgInput,borderRadius:10,padding:4}}>
        {[["labs","ð¬ Exames Laboratoriais"],["controles","ð Controles 24h"]].map(([id,label])=>(
      ...
      {tabela==="labs" && (datas.length === 0 ? (
        <div style={{padding:40,textAlign:"center",color:T.text3,fontSize:13}}>
          Nenhum dado ainda. Cole um print na aba ð¤ ou adicione um dia manualmente.
        </div>
      ) : (
        <div style={{overflowX:"auto",borderRadius:10,border:`1px solid ${T.borderTable}`}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead>
              <tr>
                <th style={{...thStyle(false),textAlign:"left",minWidth:155,padding:"8px 12px",position:"sticky",left:0,zIndex:2,background:T.bgTableHead}}>ParÃ¢metro</th>
                ...
                {datas.map(d=>(
                  <th key={d} style={thStyle(isHoje(d))}>
                    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:1}}>
                      ...
                      {!isHoje(d)&&<button onClick={()=>removerColuna(d)} style={{background:"none",border:"none",color:T.text4,cursor:"pointer",fontSize:9,padding:0}}>â</button>}
                    </div>
                  </th>
                ))}
              </tr>
              ...
              {extrasKeys.length > 0 && (
                <React.Fragment>
                  ...
                  {extrasKeys.map(k=>{
                    ...
                    const CATS_LAB = [
                      {k:"",        label:"â Sem categoria â", cor:"#475569"},
                      {k:"hb",      label:"ð©¸ HematolÃ³gico",    cor:"#f87171"},
                      {k:"cr",      label:"ð« Renal/MetabÃ³lico", cor:"#34d399"},
                      {k:"tgo",     label:"ð« Hepatograma",      cor:"#fb923c"},
                      {k:"trop",    label:"â¤ï¸ CardÃ­aco",         cor:"#f87171"},
                      {k:"po2",     label:"ð« Gasometria",       cor:"#38bdf8"},
                      {k:"he",      label:"ð´ Infeccioso",       cor:"#f59e0b"},
                    ];
                    return (
                      <tr key={k}
                        onMouseEnter={e=>e.currentTarget.style.background=T.bgCardHover}
                        ...
                        <td style={{...tdBase,padding:"4px 8px 4px 12px",textAlign:"left",position:"sticky",left:0,background:T.bgTableSticky}}>
                          <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                            ...
                            <button onClick={()=>{
                              ...
                              onChange(novo);
                            }} style={{background:"none",border:"none",color:"#334155",cursor:"pointer",fontSize:10,padding:"0 2px"}} title="Remover exame">â</button>
                          </div>
                        </td>
                        <td style={{...tdBase,fontSize:10,color:T.text3,fontFamily:mono,position:"sticky",left:155,background:T.bgTableSticky}}>â</td>
                        {datas.map(d=>{
                          const ativo=isHoje(d);
                          const raw = data[d]?.[k] || "";
                          return (
      ...
      <div style={{marginTop:8,fontSize:11,color:"#475569",display:"flex",gap:16,flexWrap:"wrap"}}>
        {tabela==="labs" && <>
          <span style={{color:"#34d399"}}>â¼ verde = queda</span>
          <span style={{color:"#f87171"}}>â² vermelho = subida</span>
        </>}
        <span>Â· Clique para editar Â· â remove a coluna do dia</span>
      </div>
      ...
      {tabela==="controles" && (
        datas.length === 0 ? (
          ...
          <div>
            {/* BotÃ£o para adicionar dreno/SNG/evac opcional */}
                ...
                {GRUPOS_CONTROLES.map(({grupo,params})=>(
                  <React.Fragment key={grupo}>
                    ...
                    {params.map(({key,label,unit})=>(
                      ...
                      {/* BalanÃ§o acumulado â calculado automaticamente logo abaixo do BH 24h */}
                      {key==="c24_bh" && (
                        <tr style={{opacity:0.80}}>
                          <td style={{...tdBase,padding:"4px 12px",fontSize:11,color:"#a78bfa",textAlign:"left",position:"sticky",left:0,background:T.bgTableSticky,fontStyle:"italic"}}>â³ BalanÃ§o Acumulado (auto)</td>
                          ...
                          {datas.map(d=>{
                            ...
                            // Calcula acumulado somando todos os c24_bh atÃ© esta data
                      ...
                      {/* DÃ©bito urinÃ¡rio calculado â logo abaixo da Diurese */}
                      {key==="c24_diur" && parseFloat(leito.peso) > 0 && (
                        <tr style={{opacity:0.75}}>
                          <td style={{...tdBase,padding:"4px 12px",fontSize:11,color:T.text3,textAlign:"left",position:"sticky",left:0,background:T.bgTableSticky,fontStyle:"italic"}}>â³ DÃ©bito urinÃ¡rio</td>
                      ...
                      {/* Kcal e ptn calculados â logo abaixo do Vol. Dieta */}
                      {key==="c24_diet_vol" && (() => {
                        ...
                        const rows = [
                          { lbl:"â³ Kcal recebida", unit:"kcal", calc:(vol)=>(vol*dietaSel.kcalML).toFixed(0), meta:metaAbs?.kcal, cor:(v,m)=>m?(v/m>=0.8?"#34d399":"#f87171"):"#94a3b8" },
                          { lbl:"â³ Ptn recebida",  unit:"g",    calc:(vol)=>(vol*dietaSel.ptnML ).toFixed(1), meta:metaAbs?.ptn,  cor:(v,m)=>m?(v/m>=0.8?"#34d399":"#f87171"):"#94a3b8" },
                        ];
                        return rows.map(row=>(
                  ...
                  return (
                    <React.Fragment>
                      <tr><td colSpan={2+datas.length} style={{padding:"7px 12px",fontSize:10,fontWeight:700,color:"#34d399",background:T.bgTableGroup,fontFamily:mono,letterSpacing:1.5,borderBottom:`1px solid ${T.borderTableRow}`}}>
                        ð§ Drenos / SNG / EvacuaÃ§Ãµes (opcionais)
                      </td></tr>
                      {drenoKeys.map(k=>{
                          ...
                          <tr key={k}
                            ...
                            <td style={{...tdBase,padding:"4px 12px",fontSize:12,color:"#34d399",textAlign:"left",position:"sticky",left:0,background:T.bgTableSticky,display:"flex",alignItems:"center",gap:6}}>
                              ...
                              <button title="Remover linha" onClick={()=>{
                                ...
                                onChange(novo);
                              }} style={{background:"none",border:"none",color:T.text3,cursor:"pointer",fontSize:10,padding:0}}>â</button>
                            </td>
...
function aplicarIA(dadosIA) {
  ...
  return {
    nEF:    s["NeurolÃ³gico"]             || "",
    cvEF:   s["HemodinÃ¢mico"]            || "",
    cv24h:  s["HemodinÃ¢mico"]            || "",
    reVM:   s["RespiratÃ³rio"]            || "",
    re24h:  s["RespiratÃ³rio"]            || "",
    rmLabs: s["Renal/MetabÃ³lico"]        || "",
    rm24h:  s["Renal/MetabÃ³lico"]        || "",
    tgEF:   s["Gastrointestinal"]        || "",
    heLabs: s["HematolÃ³gico/Infeccioso"] || "",
  };
}
...
function EvolucaoEditor({ leito, campos, onCampoEdit, config={}, tabelaHoje={} }) {
  ...
  const txtCv = () => {
    ...
    if(get("cvPerf")) p.push(`- PerfusÃ£o: ${get("cvPerf")}`);
    if(get("cvObs"))  p.push(`*${get("cvObs")}`);
    return p.join("\n");
  };
  const txtRes = () => {
    const p=[];
    if(get("reVM"))    p.push(`- VentilaÃ§Ã£o: ${get("reVM")}`);
  ...
  // Visibilidade dos campos opcionais/adicionÃ¡veis â persistida em campos._vis_
  ...
  // Refs dinÃ¢micos para campos extras adicionados
  ...
  // ââ txt funÃ§Ãµes completas (incluem opcionais/adicionÃ¡veis) ââ
  const txtNFull = () => {
    const p=[];
    if(get("nEF"))    p.push(`- EF: ${get("nEF")}`);
    if(get("nSeda"))  p.push(`- P: ${get("nSeda")}`);
  ...
  const gerarImpressao = async () => {
    ...
    try {
      ...
      const r = await fetch("/api/analyze", {
        ...
        body: JSON.stringify({
          ...
          dados: {
            ...
            diagnostico: leito.diagnostico||"nÃ£o informado",
            hda: g("hda")||"nÃ£o preenchida",
            ...
            problemas: g("probAtivos")||"nÃ£o listados",
          }
        })
      });
      const data = await r.json();
      ...
      if (!txt) throw new Error("Resposta vazia â tente novamente");
  ...
  return (
    <div>
      {/* ââ CabeÃ§alho clÃ­nico (pills) ââ */}
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
        ...
        {vc6&&<Pill label="VC 6Ã" value={vc6} unit="mL" color="#34d399"/>}
        ...
        {ativos.map((a,i)=>{
          ...
          return <span key={i} style={{padding:"4px 10px",borderRadius:20,fontSize:11,fontFamily:mono,color:al?"#f87171":"#64748b",background:al?"rgba(248,113,113,0.1)":"rgba(255,255,255,0.04)",border:`1px solid ${al?"rgba(248,113,113,0.3)":"rgba(255,255,255,0.08)"}`}}>{a.icone} D{dd}{al?" â ï¸":""}</span>;
        })}
      </div>

      {/* ââ Legenda + limpar ââ */}
      <div style={{display:"flex",gap:16,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,fontSize:11,color:"#64748b"}}>
          ...
          Dia anterior â edite para atualizar
        </div>
        <button onClick={()=>{
          if(confirm("Limpar toda a evoluÃ§Ã£o deste leito?")) {
      ...
      {/* ââ HDA ââ */}
      <SysB id="hda" sigla="== HDA:" label="HistÃ³ria da DoenÃ§a Atual" color={"#c084fc"} txtFn={()=>get("hda")}
        camposVisiveis={vis} setCamposVisiveis={setCamposVis}
        opcionais={[]} adicionaveis={[]}>
        <Row><Col><FL>HISTÃRIA â resumo clÃ­nico para passagem de caso</FL>
      ...
      {/* ââ Contexto: DiagnÃ³stico Â· Procedimentos Â· Dispositivos ââ */}
      {(leito.diagnostico||(leito.procedimentos||[]).length>0||ativos.length>0) && (
        <div style={{marginBottom:10,border:"1px solid rgba(255,255,255,0.07)",borderRadius:10,overflow:"hidden"}}>
          <div style={{background:"rgba(255,255,255,0.03)",padding:"10px 14px",display:"flex",alignItems:"center",gap:8}}>
            ...
            <span style={{fontSize:12,color:"#475569",fontWeight:400}}>DiagnÃ³stico Â· Procedimentos Â· Dispositivos</span>
          </div>
          <div style={{padding:"12px 14px",borderTop:"1px solid rgba(255,255,255,0.05)",display:"flex",flexDirection:"column",gap:8}}>
            {leito.diagnostico && (
              <div style={{display:"flex",alignItems:"baseline",gap:8}}>
                <span style={{fontSize:10,color:"#64748b",fontFamily:mono,letterSpacing:1,flexShrink:0}}>DIAGNÃSTICO</span>
              ...
              <div style={{display:"flex",alignItems:"baseline",gap:8,flexWrap:"wrap"}}>
                ...
                <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                  {(leito.procedimentos||[]).map(p=>{
                    const po=Math.floor((new Date()-new Date(p.data+"T00:00:00"))/86400000);
                    const cor=po<=0?"#f87171":po<=3?"#fb923c":po<=7?"#fbbf24":"#34d399";
                    return <span key={p.id} style={{fontSize:12,fontFamily:mono,fontWeight:700,color:cor,background:`${cor}18`,border:`1px solid ${cor}44`,borderRadius:6,padding:"2px 10px"}}>{p.nome} Â· {po===0?"POI":`PO${po}`}</span>;
                  })}
                </div>
              </div>
            )}
                ...
                <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                  {ativos.map((a,i)=>{
                    ...
                    return <span key={i} style={{fontSize:11,fontFamily:mono,color:al?"#f87171":"#94a3b8",background:al?"rgba(248,113,113,0.08)":"rgba(255,255,255,0.04)",border:`1px solid ${al?"rgba(248,113,113,0.25)":"rgba(255,255,255,0.08)"}`,borderRadius:6,padding:"2px 10px"}}>
                      {a.icone} {a.label}{a.disp.site?` Â· ${a.disp.site}`:""} D{dd}{al?" â ï¸":""}
                    </span>;
                  })}
                </div>
              </div>
      ...
      <SysB id="n" sigla="== N:" label="NeurolÃ³gico" color={"#a78bfa"} txtFn={txtNFull}
        camposVisiveis={vis} setCamposVisiveis={setCamposVis}
        ...
        <Row><Col><FL>EF â GCS Â· RASS Â· Pupilas Â· DÃ©ficit</FL><TA fieldRef={refs.nEF} defaultValue={campos.nEF} isAntigo={isAntigo("nEF")} sugestao="GCS 12T (AO4 RV2 RM6) / RASS 0 / Pupilas isofotorreagentes 2-2" rows={2} fieldName="nEF" onBlurSave={salvar}/></Col></Row>
        <Row>
          <Col><FL>P â SEDAÃÃO</FL><TA fieldRef={refs.nSeda} defaultValue={campos.nSeda} isAntigo={isAntigo("nSeda")} sugestao="Precedex 10ml/h (0,57 mcg/kg/h) + Quetiapina 150mg/d" rows={2} fieldName="nSeda" onBlurSave={salvar}/></Col>
          <Col><FL>A â ANALGESIA</FL><TA fieldRef={refs.nAnalg} defaultValue={campos.nAnalg} isAntigo={isAntigo("nAnalg")} sugestao="Metadona 22,5mg/d + Cetamina 5ml/h" rows={2} fieldName="nAnalg" onBlurSave={salvar}/></Col>
        </Row>
        ...
        {vis["add_n_interconsulta"]&&<Row><Col><FL>INTERCONSULTA</FL><TA fieldRef={ExtraRef("add_n_interconsulta")} defaultValue={campos["add_n_interconsulta"]||""} sugestao="Neurologia 29/04: aguarda avaliaÃ§Ã£o" rows={1} fieldName="add_n_interconsulta" onBlurSave={salvar}/></Col></Row>}
        {vis["add_n_exames"]&&<Row><Col><FL>EXAMES COMPLEMENTARES</FL><TA fieldRef={ExtraRef("add_n_exames")} defaultValue={campos["add_n_exames"]||""} sugestao="RM crÃ¢nio solicitada / EEG pendente" rows={1} fieldName="add_n_exames" onBlurSave={salvar}/></Col></Row>}
        {vis["add_n_pocus"]&&<Row><Col><FL>POCUS</FL><TA fieldRef={ExtraRef("add_n_pocus")} defaultValue={campos["add_n_pocus"]||""} sugestao="POCUS 29/04: sem alteraÃ§Ãµes" rows={1} fieldName="add_n_pocus" onBlurSave={salvar}/></Col></Row>}
        {vis["nObs"]&&<Row><Col><FL>* OBSERVAÃÃO</FL><TA fieldRef={refs.nObs} defaultValue={campos.nObs} isAntigo={isAntigo("nObs")} sugestao="AvaliaÃ§Ã£o neuro 21/04: Ã¡rea hipodensa em tronco â aguarda RM" rows={1} fieldName="nObs" onBlurSave={salvar}/></Col></Row>}
      </SysB>

      <SysB id="cv" sigla="== Cv:" label="Cardiovascular" color={"#f87171"} txtFn={txtCvFull}
        camposVisiveis={vis} setCamposVisiveis={setCamposVis}
        opcionais={[{key:"cvMed",label:"MedicaÃ§Ãµes"},{key:"cvObs",label:"Obs"}]}
        ...
        <Row><Col><FL>EF â Estabilidade Â· Ritmo Â· Bulhas</FL><TA fieldRef={refs.cvEF} defaultValue={campos.cvEF} isAntigo={isAntigo("cvEF")} sugestao="Hemodinamicamente estÃ¡vel, sem DVA. RCR, 2T, BNF SS." rows={2} fieldName="cvEF" onBlurSave={salvar}/></Col></Row>
        <Row>
          <Col><FL>24h â FC / PAM (mÃ­n-mÃ¡x)</FL><TA fieldRef={refs.cv24h} defaultValue={campos.cv24h} isAntigo={isAntigo("cv24h")} sugestao="FC 109 - 58 / PAM 121 - 58" rows={1} fieldName="cv24h" onBlurSave={salvar}/></Col>
          <Col><FL>DVA â Droga + vazÃ£o + dose</FL><TA fieldRef={refs.cvDVA} defaultValue={campos.cvDVA} isAntigo={isAntigo("cvDVA")} sugestao="Nora 5ml/h (0,08 mcg/kg/min)" rows={1} fieldName="cvDVA" onBlurSave={salvar}/></Col>
        </Row>
        {vis["cvMed"]&&<Row><Col><FL>P â MEDICAÃÃES CV</FL><TA fieldRef={refs.cvMed} defaultValue={campos.cvMed} isAntigo={isAntigo("cvMed")} sugestao="Atenolol 25mg / Furosemida 40mg/d" rows={1} fieldName="cvMed" onBlurSave={salvar}/></Col></Row>}
        <Row><Col><FL>PerfusÃ£o â TEC Â· Lactato</FL><TA fieldRef={refs.cvPerf} defaultValue={campos.cvPerf} isAntigo={isAntigo("cvPerf")} sugestao="TEC 2 seg / Lactato 12 > 22" rows={1} fieldName="cvPerf" onBlurSave={salvar}/></Col></Row>
        ...
        {vis["add_cv_exames"]&&<Row><Col><FL>EXAMES COMPLEMENTARES</FL><TA fieldRef={ExtraRef("add_cv_exames")} defaultValue={campos["add_cv_exames"]||""} sugestao="ECG 29/04: RS, sem alteraÃ§Ãµes" rows={1} fieldName="add_cv_exames" onBlurSave={salvar}/></Col></Row>}
        ...
        {vis["cvObs"]&&<Row><Col><FL>* OBSERVAÃÃO</FL><TA fieldRef={refs.cvObs} defaultValue={campos.cvObs} isAntigo={isAntigo("cvObs")} sugestao="Eco beira-leito amanhÃ£" rows={1} fieldName="cvObs" onBlurSave={salvar}/></Col></Row>}
      </SysB>

      <SysB id="res" sigla="== Res:" label="RespiratÃ³rio" color={"#38bdf8"} txtFn={txtResFull}
        camposVisiveis={vis} setCamposVisiveis={setCamposVis}
        opcionais={[{key:"rePocus",label:"POCUS"},{key:"reObs",label:"Obs"}]}>
        <Row><Col><FL>VentilaÃ§Ã£o â Modo Â· PS Â· PEEP Â· FiO2 Â· Pocc</FL><TA fieldRef={refs.reVM} defaultValue={campos.reVM} isAntigo={isAntigo("reVM")} sugestao="TQT em VM modo PSV, PS12 PEEP6 Fi30% / Pocc 7" rows={2} fieldName="reVM" onBlurSave={salvar}/></Col></Row>
        <Row>
          <Col><FL>EF â Ausculta</FL><TA fieldRef={refs.reEF} defaultValue={campos.reEF} isAntigo={isAntigo("reEF")} sugestao="MV + bilateralmente c/ roncos" rows={1} fieldName="reEF" onBlurSave={salvar}/></Col>
          <Col><FL>24h â FR / Sat (mÃ­n-mÃ¡x)</FL><TA fieldRef={refs.re24h} defaultValue={campos.re24h} isAntigo={isAntigo("re24h")} sugestao="FR 41 - 20 / Sat 96 - 92" rows={1} fieldName="re24h" onBlurSave={salvar}/></Col>
        </Row>
        ...
        {vis["rePocus"]&&<Row><Col><FL>POCUS â Data Â· Achados</FL><TA fieldRef={refs.rePocus} defaultValue={campos.rePocus} isAntigo={isAntigo("rePocus")} sugestao="22/04: ExcursÃ£o 0,87 / Fen 12%" rows={1} fieldName="rePocus" onBlurSave={salvar}/></Col></Row>}
        {vis["reObs"]&&<Row><Col><FL>* OBSERVAÃÃO</FL><TA fieldRef={refs.reObs} defaultValue={campos.reObs} isAntigo={isAntigo("reObs")} sugestao="Tentar reduzir PS amanhÃ£" rows={1} fieldName="reObs" onBlurSave={salvar}/></Col></Row>}
      </SysB>

      <SysB id="reme" sigla="== ReMe:" label="Renal / MetabÃ³lico" color={"#34d399"} txtFn={txtReMeFull}
        camposVisiveis={vis} setCamposVisiveis={setCamposVis}
        opcionais={[{key:"rmTRS",label:"TRS"},{key:"rmObs",label:"Obs"}]}
        adicionaveis={[{key:"interconsulta",label:"Interconsulta"}]}>
        <Row><Col><FL>24h â HD Â· BH</FL><TA fieldRef={refs.rm24h} defaultValue={campos.rm24h} isAntigo={isAntigo("rm24h")} sugestao="HD 3000 / BH +1084 > +1508" rows={1} fieldName="rm24h" onBlurSave={salvar}/></Col></Row>
        ...
        <Row><Col><FL>Labs â Cr Â· Ur Â· K Â· Na Â· Cai Â· Mg Â· P</FL><TA fieldRef={refs.rmLabs} defaultValue={campos.rmLabs} isAntigo={isAntigo("rmLabs")} sugestao="Cr 1,56 > 1,27 / Ur 66 > 47 / K 4,2 > 4,1 / Na 143 > 141" rows={2} fieldName="rmLabs" onBlurSave={salvar}/></Col></Row>
        {vis["add_reme_interconsulta"]&&<Row><Col><FL>INTERCONSULTA</FL><TA fieldRef={ExtraRef("add_reme_interconsulta")} defaultValue={campos["add_reme_interconsulta"]||""} sugestao="Nefrologia 29/04: avaliou TRS â manter CRRT" rows={1} fieldName="add_reme_interconsulta" onBlurSave={salvar}/></Col></Row>}
        {vis["rmObs"]&&<Row><Col><FL>* OBSERVAÃÃO</FL><TA fieldRef={refs.rmObs} defaultValue={campos.rmObs} isAntigo={isAntigo("rmObs")} sugestao="Repor K se < 3,5" rows={1} fieldName="rmObs" onBlurSave={salvar}/></Col></Row>}
      </SysB>
      ...
      <SysB id="tgi" sigla="== TGI:" label="Gastrointestinal" color={"#fb923c"} txtFn={txtTGIFull}
        ...
        {leito.dieta?.tipo&&<div style={{padding:"6px 10px",background:"rgba(251,146,60,0.07)",border:"1px solid rgba(251,146,60,0.2)",borderRadius:6,fontSize:11,color:"#fb923c",marginBottom:8}}>
          ð½ <strong>{leito.dieta.tipo}</strong>{leito.dieta.formula&&` â ${leito.dieta.formula}`}{leito.dieta.vazao&&` @ ${leito.dieta.vazao} mL/h`}
          {(()=>{
            ...
            return <span style={{marginLeft:8,color:"#94a3b8"}}>Â· Kcal: <strong style={{color:pctKcal>=80?"#34d399":"#f87171"}}>{pctKcal}%</strong> Â· Ptn: <strong style={{color:pctPtn>=80?"#34d399":"#f87171"}}>{pctPtn}%</strong></span>;
          })()}
        </div>}
        <Row>
          <Col><FL>EF â Abdome</FL><TA fieldRef={refs.tgEF} defaultValue={campos.tgEF} isAntigo={isAntigo("tgEF")} sugestao="AbdÃ´men globoso, flÃ¡cido, indolor Ã  palpaÃ§Ã£o." rows={2} fieldName="tgEF" onBlurSave={salvar}/></Col>
          <Col><FL>24h â Dex Â· EvacuaÃ§Ã£o</FL><TA fieldRef={refs.tg24h} defaultValue={campos.tg24h} isAntigo={isAntigo("tg24h")} sugestao="Dex 105 - 167 | Ãºltima evacuaÃ§Ã£o 21/04" rows={2} fieldName="tg24h" onBlurSave={salvar}/></Col>
        </Row>
        <Row><Col><FL>Labs â TGO Â· TGP Â· Bili Â· FA Â· GGT Â· Alb</FL><TA fieldRef={refs.tgLabs} defaultValue={campos.tgLabs} isAntigo={isAntigo("tgLabs")} sugestao="TGO 45 / TGP 32 / BT 1.2 / Alb 2.8" rows={1} fieldName="tgLabs" onBlurSave={salvar}/></Col></Row>
        {vis["add_tgi_interconsulta"]&&<Row><Col><FL>INTERCONSULTA</FL><TA fieldRef={ExtraRef("add_tgi_interconsulta")} defaultValue={campos["add_tgi_interconsulta"]||""} sugestao="Gastro 29/04: endoscopia nÃ£o indicada no momento" rows={1} fieldName="add_tgi_interconsulta" onBlurSave={salvar}/></Col></Row>}
        ...
        {vis["tgObs"]&&<Row><Col><FL>* OBSERVAÃÃO</FL><TA fieldRef={refs.tgObs} defaultValue={campos.tgObs} isAntigo={isAntigo("tgObs")} sugestao="Omeprazol para LAMG" rows={1} fieldName="tgObs" onBlurSave={salvar}/></Col></Row>}
      </SysB>

      <SysB id="he" sigla="== He:" label="HematolÃ³gico" color={"#f59e0b"} txtFn={txtHeFull}
        camposVisiveis={vis} setCamposVisiveis={setCamposVis}
        opcionais={[{key:"heProf",label:"Profilaxias"},{key:"heObs",label:"Obs"}]}
        adicionaveis={[{key:"interconsulta",label:"Interconsulta"},{key:"exames",label:"Exames Compl."}]}>
        <Row>
          <Col><FL>Temperatura â mÃ­n Â· mÃ¡x</FL><TA fieldRef={refs.heTemp} defaultValue={campos.heTemp} isAntigo={isAntigo("heTemp")} sugestao="37,2 - 36,2" rows={1} fieldName="heTemp" onBlurSave={salvar}/></Col>
          {vis["heProf"]&&<Col><FL>** Profilaxias / TEV</FL><TA fieldRef={refs.heProf} defaultValue={campos.heProf} isAntigo={isAntigo("heProf")} sugestao="HNF 5kUI 12/12h / Bactrim + Ãc fÃ³lico" rows={1} fieldName="heProf" onBlurSave={salvar}/></Col>}
        </Row>
        <Row><Col><FL>Labs â Hb Â· Leuco Â· BastÃµes Â· Plaq</FL><TA fieldRef={refs.heLabs} defaultValue={campos.heLabs} isAntigo={isAntigo("heLabs")} sugestao="7,6 > 7,5 / Leuco 21k > 14k / BastÃµes 5% > 4% / Plaq 191k > 251k" rows={2} fieldName="heLabs" onBlurSave={salvar}/></Col></Row>
        {vis["add_he_interconsulta"]&&<Row><Col><FL>INTERCONSULTA</FL><TA fieldRef={ExtraRef("add_he_interconsulta")} defaultValue={campos["add_he_interconsulta"]||""} sugestao="Hematologia 29/04: sem indicaÃ§Ã£o de transfusÃ£o" rows={1} fieldName="add_he_interconsulta" onBlurSave={salvar}/></Col></Row>}
        ...
        {vis["heObs"]&&<Row><Col><FL>* OBSERVAÃÃO</FL><TA fieldRef={refs.heObs} defaultValue={campos.heObs} isAntigo={isAntigo("heObs")} sugestao="Aguarda cultura / BAAR negativo" rows={1} fieldName="heObs" onBlurSave={salvar}/></Col></Row>}
      </SysB>
      ...
      <SysB id="in" sigla="== In:" label="Infeccioso / Dispositivos" color={"#94a3b8"} txtFn={txtInFull}
        ...
        {ativos.length>0&&<div style={{padding:"6px 10px",background:"rgba(148,163,184,0.07)",border:"1px solid rgba(148,163,184,0.15)",borderRadius:6,fontSize:11,color:"#94a3b8",marginBottom:8}}>
          ð {ativos.map(a=>{const dd=Math.floor((new Date()-new Date(a.disp.data+"T00:00:00"))/86400000);return `${a.label}${a.disp.site?` (${a.disp.site})`:""} D${dd}`;}).join(" / ")}
        </div>}
        {vis["inProf"]&&<Row><Col><FL>Profilaxias / Outros medicamentos</FL><TA fieldRef={refs.heMed} defaultValue={campos.heMed} isAntigo={isAntigo("heMed")} sugestao="Bactrim + Ãc fÃ³lico / Eritropoietina 4000 UI 48/48h" rows={2} fieldName="heMed" onBlurSave={salvar}/></Col></Row>}
        <Row><Col><FL>AntibiÃ³ticos â nome + perÃ­odo</FL><TA fieldRef={refs.heAtb} defaultValue={campos.heAtb} isAntigo={isAntigo("heAtb")} sugestao={"- Meropenem + Vanco (15/04 - 22/04)\n- Tazocin + Claritromicina (21/03-27/03/2026)"} rows={3} fieldName="heAtb" onBlurSave={salvar}/></Col></Row>
        <Row><Col><FL>ð§« Culturas â material Â· data Â· resultado</FL><TA fieldRef={refs.heCulturas} defaultValue={campos.heCulturas} isAntigo={isAntigo("heCulturas")} sugestao={"- Hemocultura 23/04: pendente\n- Urinocultura 22/04: E.coli ESBL"} rows={3} fieldName="heCulturas" onBlurSave={salvar}/></Col></Row>
        ...
        {vis["inObs"]&&<Row><Col><FL>* OBSERVAÃÃO</FL><TA fieldRef={ExtraRef("inObs")} defaultValue={campos["inObs"]||""} sugestao="ReavaliaÃ§Ã£o com culturas em 48h" rows={1} fieldName="inObs" onBlurSave={salvar}/></Col></Row>}
      </SysB>

      <SysB id="prob" sigla="ð Prob:" label="Problemas" color="#94a3b8" txtFn={txtProblemas}>
        <Row>
          <Col>
            <FL>ð´ Problemas Ativos</FL>
          ...
          <Col>
            <FL>â Problemas Resolvidos</FL>
            <TA fieldRef={refs.probResolvidos} defaultValue={campos.probResolvidos} isAntigo={isAntigo("probResolvidos")} sugestao={"1. Choque sÃ©ptico (resolvido D5)\n2. Acidose metabÃ³lica"} rows={4} fieldName="probResolvidos" onBlurSave={salvar}/>
          </Col>
        </Row>
      </SysB>
      ...
      <div style={{marginBottom:10,border:`1px solid rgba(56,189,248,0.2)`,borderRadius:10,overflow:"hidden",background:"rgba(56,189,248,0.02)"}}>
        <div style={{display:"flex",alignItems:"center",background:"rgba(56,189,248,0.05)",padding:"10px 14px",gap:8}}>
          ...
          <span style={{fontSize:12,fontWeight:700,color:"#38bdf8",fontFamily:mono,letterSpacing:1.5}}>== ImpressÃ£o:</span>
          <span style={{fontSize:12,color:"#475569",fontWeight:400}}>Resumo automÃ¡tico para passagem de caso</span>
          <div style={{marginLeft:"auto",display:"flex",gap:6}}>
            <button onClick={gerarImpressao} disabled={impLoading}
              ...
              {impLoading ? <><span style={{animation:"spin 1s linear infinite",display:"inline-block"}}>â³</span> Gerandoâ¦</> : impGerado ? "â Gerado!" : "â¨ Gerar com IA"}            </button>
            <button onClick={()=>{
              ...
              {copiado.impressao ? "â Copiado" : "ð Copiar"}
            </button>
          </div>
        </div>
        <div style={{padding:"12px 14px",borderTop:"1px solid rgba(56,189,248,0.1)"}}>
          <div style={{fontSize:10,color:"#64748b",fontFamily:mono,letterSpacing:1,marginBottom:5}}>
            IMPRESSÃO CLÃNICA â clique em â¨ Gerar com IA para gerar narrativa Â· edite Ã  vontade apÃ³s geraÃ§Ã£o
          </div>
          ...
          <textarea ref={refs.impressao} defaultValue={campos.impressao||""} rows={6}
            ...
            placeholder={"Clique em â¨ Gerar com IA para criar a narrativa clÃ­nica automaticamente.\n\nOu escreva diretamente aqui a sua impressÃ£o do quadro para passagem de caso."}
        ...
        {copiado.tudo?"â EvoluÃ§Ã£o completa copiada!":"ð Copiar evoluÃ§Ã£o completa"}
      </button>
    </div>
  );
}

// ââ FerramentasPanel ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function FerramentasPanel() {
  const LINKS = [
    {
      emoji:"ð«", cor:"#38bdf8", bg:"rgba(56,189,248,0.08)", borda:"rgba(56,189,248,0.25)",
      titulo:"Checklist de IOT", desc:"Passo a passo para intubaÃ§Ã£o e via aÃ©rea difÃ­cil",
    ...
    {
      emoji:"ð", cor:"#f59e0b", bg:"rgba(245,158,11,0.08)", borda:"rgba(245,158,11,0.25)",
      titulo:"Profilaxia AntibiÃ³tica CirÃºrgica", desc:"Hospital SÃ£o Paulo / UNIFESP â Rev. 2024 Â· Ortopedia, Neuro, GI, Gineco, CardÃ­aca, Vascular, Transplante...",
      href:"/atb_profilaxia.pdf",
      label:"Abrir PDF"
    },
    {
      emoji:"ð«", cor:"#f87171", bg:"rgba(248,113,113,0.08)", borda:"rgba(248,113,113,0.25)",
      titulo:"Protocolo PÃ³s-op Transplante HepÃ¡tico", desc:"Rotina de atendimento, prescriÃ§Ã£o, monitorizaÃ§Ã£o e complicaÃ§Ãµes no pÃ³s-operatÃ³rio",
      href:"/tx_hepatico.pdf",
      label:"Abrir PDF"
    },
    {
      emoji:"ð§®", cor:"#a78bfa", bg:"rgba(167,139,250,0.08)", borda:"rgba(167,139,250,0.25)",
      titulo:"MDCalc", desc:"Calculadoras mÃ©dicas, escores (APACHE, SAPS, SOFA, Glasgow...)",
  ...
  return (
    <div style={{padding:"24px", maxWidth:"800px", margin:"0 auto", width:"100%"}}>
      <div style={{marginBottom:24}}>
        <div style={{fontSize:22, fontWeight:700, color:"#e2e8f0", marginBottom:6}}>ð Links & Protocolos</div>
        <div style={{fontSize:13, color:"#64748b"}}>Acesso rÃ¡pido a protocolos, checklists e guias da unidade.</div>
      </div>
...
// ââ MetasPanel ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function MetasPanel({ metas, onChange }) {
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:8}}>
        <input value={nova} onChange={e=>setNova(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add(nova)} placeholder="Nova meta ou pendÃªnciaâ¦"
      ...
      <button onClick={()=>setShow(s=>!s)} style={{width:"100%",padding:"7px",background:"transparent",border:"1px dashed rgba(255,255,255,0.1)",borderRadius:8,color:"#64748b",fontSize:12,cursor:"pointer",marginBottom:12}}>
        {show?"â² Ocultar sugestÃµes":"â¼ Ver sugestÃµes de metas comuns"}
      </button>
      ...
      {metas.length===0 && <div style={{textAlign:"center",padding:24,color:"#334155",fontSize:13}}>Nenhuma meta cadastrada para este plantÃ£o</div>}
      {metas.map(m=>(
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {["pendente","andamento","cumprido"].map(st=>(
                <button key={st} onClick={()=>onChange(metas.map(x=>x.id===m.id?{...x,status:st}:x))}
                  ...
                  {st==="pendente"?"â Pendente":st==="andamento"?"â Andamento":"â Cumprido"}
                </button>
              ))}
            </div>
          </div>
          <button onClick={()=>onChange(metas.filter(x=>x.id!==m.id))} style={{background:"none",border:"none",color:"#475569",cursor:"pointer",fontSize:16,padding:2}}>â</button>
        </div>
      ))}
    </div>
  );
...
// ââ LeitoCard âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  return (
    <div style={{cursor:"pointer",borderRadius:12,padding:"14px 16px",background:selecionado?T.bgSel:T.bgCard,border:`1.5px solid ${selecionado?T.accent:T.border}`,transition:"all 0.2s",marginBottom:8,boxShadow:selecionado?"none":T.shadowCard}} onClick={e=>{if(!editingNome) onClick();}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:4}}>
          ...
          {!editingNome && (
            <button onClick={e=>{e.stopPropagation();setEditingNome(true);setNomeTemp(leito.nome);}}
              ...
              style={{background:"none",border:"none",color:T.text4,cursor:"pointer",fontSize:11,padding:"0 2px",lineHeight:1}}>âï¸</button>
          )}
          {onRemove && (
            <button onClick={e=>{e.stopPropagation();if(confirm(`Remover ${leito.nome}?`))onRemove();}}
              title="Remover leito"
              style={{background:"none",border:"none",color:T.text4,cursor:"pointer",fontSize:11,padding:"0 2px",lineHeight:1}}>ðï¸</button>
          )}
        </div>
      </div>
      {vago ? <div style={{fontSize:13,color:T.textDim,marginTop:4,fontStyle:"italic"}}>Vago</div> : <>
        ...
        {(leito.peso||leito.altura)&&<div style={{fontSize:11,color:T.text3,marginTop:3}}>{leito.peso?`${leito.peso} kg`:""}{leito.peso&&leito.altura?" Â· ":""}{leito.altura?`${leito.altura} cm`:""}</div>}
        {(leito.procedimentos||[]).length>0&&(
        ...
        {(() => {
          ...
          return temAlerta ? <div style={{marginTop:5,fontSize:10,color:"#f87171",fontFamily:mono}}>â ï¸ Dispositivo p/ revisÃ£o</div> : null;
        })()}
      </>}
    </div>
  );
...
// ââ LoginScreen âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function LoginScreen({ onLogin }) {
  ...
  const handleSetup = async () => {
    ...
    if (senha !== confirma) { setErro("As senhas nÃ£o coincidem."); return; }
    setLoading(true); setErro("");
    try {
      const hash = await sha256(senha);
  ...
  if (mode === null) return (
    <div style={{minHeight:"100vh",background:"#080f0a",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Sora',sans-serif"}}>
      ...
      <div style={{color:"#38bdf8"}}>Carregandoâ¦</div>
    </div>
  );
    ...
    <div style={{minHeight:"100vh",background:"#080f0a",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Sora','DM Sans',sans-serif"}}>
      ...
      <div style={{width:"100%",maxWidth:380,padding:32}}>
        <div style={{textAlign:"center",marginBottom:36}}>
          ...
          <div style={{fontSize:11,color:"#475569",fontFamily:"'DM Mono',monospace",letterSpacing:2,marginTop:4}}>ASSISTENTE DE EVOLUÃÃO</div>
        </div>
        <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:28}}>
          <div style={{fontSize:14,fontWeight:600,color:"#cbd5e1",marginBottom:20,textAlign:"center"}}>
            {mode==="setup" ? "ð Criar senha de acesso" : "ð Acesso restrito"}
          </div>
          <div style={{marginBottom:14}}>
          ...
          {mode==="setup" && (
            <div style={{marginBottom:14}}>
              ...
              <input type="password" value={confirma} onChange={e=>setConfirma(e.target.value)}
                onKeyDown={e=>e.key==="Enter"&&handleSetup()} placeholder="â¢â¢â¢â¢â¢â¢â¢â¢"
                style={{width:"100%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"10px 14px",color:"#e2e8f0",fontSize:14,fontFamily:"inherit"}}/>
            </div>
          )}
          ...
          <button onClick={mode==="setup"?handleSetup:handleLogin} disabled={loading||!senha}
            ...
            {loading?"Verificandoâ¦":mode==="setup"?"Criar senha e entrar":"Entrar"}
          </button>
        </div>
        {mode==="setup"&&<div style={{marginTop:16,padding:"10px 14px",background:"rgba(245,158,11,0.07)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:8,fontSize:12,color:"#fcd34d",lineHeight:1.6}}>
          ð A senha Ã© salva de forma criptografada no banco de dados. Funciona em qualquer dispositivo.
        </div>}
      </div>
    </div>
  );
...
export default function App() {
  ...
  const loadData = async () => {
    ...
    // Libera saves apenas apÃ³s load completo
  ...
  // ââ Favicon + tÃ­tulo ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  useEffect(()=>{
  ...
  // ââ SAVES manuais (chamados explicitamente, nÃ£o por useEffect) ââââââââââââââââ
  ...
  const ABAS = [
    {id:"paciente", label:"ð¤ Paciente & CÃ¡lculos"},
    {id:"tabela",   label:"ð Tabela ClÃ­nica"},
    {id:"upload",   label:"ð¤ Importar Print"},
    {id:"evolucao", label:"ð EvoluÃ§Ã£o"},
    {id:"metas",    label:"ð¯ Metas & PendÃªncias"},
  ];
  ...
  if (!appReady) return (
    <div style={{minHeight:"100vh",background:"#080f0a",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Sora',sans-serif"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Sora:wght@400;600;700&family=DM+Mono:wght@400;500&display=swap');*{box-sizing:border-box}`}</style>
      <div style={{color:"#38bdf8",fontSize:14}}>Carregandoâ¦</div>
    </div>
  );
  ...
  return (
    ...
    <div style={{minHeight:"100vh",background:T.bgPage,fontFamily:"'Sora','DM Sans',sans-serif",color:T.text1,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:"0 24px",height:56,display:"flex",alignItems:"center",borderBottom:`1px solid ${T.borderAccent}`,background:T.bgHeader,position:"sticky",top:0,zIndex:100,backdropFilter:"blur(12px)"}}>
        <button onClick={()=>setShowSidebar(s=>!s)} style={{background:"none",border:`1px solid ${T.border}`,borderRadius:6,color:T.text3,cursor:"pointer",fontSize:16,padding:"4px 8px",marginRight:14}} title="Toggle sidebar">â°</button>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <BrainLogo size={32}/>
          <div>
            <div style={{fontSize:14,fontWeight:700,letterSpacing:0.5,color:T.text1}}>UTI Evolve</div>
            <div style={{fontSize:9,color:T.accent,fontFamily:mono,letterSpacing:2}}>ASSISTENTE DE EVOLUÃÃO</div>
          </div>
        </div>
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:14}}>
          <div style={{fontSize:11,fontFamily:mono,color:saving?"#f59e0b":T.accent,display:"flex",alignItems:"center",gap:4}}>
            ...
            {saving?"Salvandoâ¦":"Salvo"} <span style={{fontSize:9,color:T.text4,fontFamily:mono}}>2026-05-10T21:15</span>
          </div>
          ...
          <button onClick={()=>setAba("config")} title="ConfiguraÃ§Ãµes" style={{background:"none",border:`1px solid ${T.border}`,borderRadius:6,color:T.text3,cursor:"pointer",fontSize:14,padding:"4px 8px"}}>âï¸</button>
        </div>
      </div>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden",height:"calc(100vh - 56px)"}}>
          ...
          {leitos.map((l, idx)=>(
            <div key={l.id} style={{display:"flex",alignItems:"stretch",gap:4,marginBottom:0}}>
              <div style={{display:"flex",flexDirection:"column",gap:2,justifyContent:"center",paddingBottom:8}}>
                ...
                }} style={{background:"none",border:"none",color:idx===0?"#1e293b":"#475569",cursor:idx===0?"default":"pointer",fontSize:10,padding:"1px 3px",lineHeight:1}}>â²</button>
                <button onClick={()=>{
                  if(idx===leitos.length-1) return;
                  setLeitos(ls=>{const n=[...ls];[n[idx],n[idx+1]]=[n[idx+1],n[idx]];salvarLeitos(n);return n;});
                }} style={{background:"none",border:"none",color:idx===leitos.length-1?"#1e293b":"#475569",cursor:idx===leitos.length-1?"default":"pointer",fontSize:10,padding:"1px 3px",lineHeight:1}}>â¼</button>
              </div>
              <div style={{flex:1}}>
        ...
        <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
          ...
          {leito.paciente && (
            <div style={{padding:"13px 28px",borderBottom:`1px solid ${T.border}`,background:T.bgCard}}>
              ...
              <div style={{fontSize:12,color:T.text3,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginTop:2}}>
                <span>{leito.diagnostico}{dias!==null&&` Â· D${dias}`}{leito.peso&&` Â· ${leito.peso} kg`}{pp&&` Â· PP ${pp} kg`}</span>
                {(leito.procedimentos||[]).map(p=>{
                  const po=Math.floor((new Date()-new Date(p.data+"T00:00:00"))/86400000);
                  const cor=po===0?"#f87171":po<=3?"#fb923c":po<=7?"#fbbf24":"#34d399";
                ...
                {[
                  ...
                  return <span key={i} style={{fontSize:10,fontFamily:mono,color:cor,background:`${cor}18`,border:`1px solid ${cor}44`,borderRadius:4,padding:"1px 7px"}}>{a.label} D{po}{po>a.alertaDias?" â ï¸":""}</span>;
                })}
              </div>
            </div>
          )}
          ...
          <div style={{flex:1,overflowY:"auto",padding:pagePadding,background:T.bgPage}}>
            {aba==="config" ? (
              <ConfigPanel config={config} onChange={c=>{setConfig(c);salvarConfig(c);}} onVoltar={()=>setAba("paciente")}/>
            ) : aba==="paciente" ? (
              <div style={{width:"100%",maxWidth:contentWide}}><PacientePanel
              ...
              <div style={{width:"100%",maxWidth:contentMedium}}>
                <div style={{marginBottom:18}}>
                  <div style={{fontSize:15,fontWeight:700,marginBottom:6,color:T.text1}}>Importar dados via imagem</div>
                  <div style={{fontSize:13,color:T.text3}}>FaÃ§a upload do print do Tasy. A IA extrai os dados e vocÃª revisa antes de aplicar na evoluÃ§Ã£o.</div>
                </div>
                <UploadAnalyzer onResult={d=>{
                  const hoje = new Date().toISOString().split("T")[0];
                  // Usa a data de coleta do exame se disponÃ­vel, senÃ£o hoje
                  ...
                  // Regex: captura nÃºmeros com vÃ­rgula OU ponto como decimal
                  ...
                  Object.assign(novos, extrair(s["HemodinÃ¢mico"]||"", [
                    ["lact",  re(`[Ll]actato[:\\s]+${NUM}`)],
                    ["trop",  re(`[Tt]roponina[:\\s]+${NUM}`)],
                    ["bnp",   re(`\\bBNP[:\\s]+${NUM}`)],
                  ]));
                  Object.assign(novos, extrair(s["Renal/MetabÃ³lico"]||"", [
                  ...
                  Object.assign(novos, extrair(s["HematolÃ³gico/Infeccioso"]||"", [
                    ["hb",    re(`\\bHb[:\\s]+${NUM}`)],
                    ["ht",    re(`\\bHt[:\\s]+${NUM}`)],
                    ["leuco", re(`[Ll]euco[citos\\s]*[:/\\s]+${NUM}`)],
                    ["neut",  re(`[Nn]eutr[Ã³o\\s]*[:/\\s]+${NUM}`)],
                    ["bast",  re(`[Bb]ast[Ãµes\\s]*[:/\\s]+${NUM}`)],
                    ["linf",  re(`[Ll]inf[Ã³citos\\s]*[:/\\s]+${NUM}`)],
                    ["plaq",  re(`[Pp]laq[uetas\\s]*[:/\\s]+${NUM}`)],
                    ["rni",   re(`\\bRNI[:\\s]+${NUM}`)],
                    ["ttpa",  re(`\\bTTPA[:\\s]+${NUM}`)],
                  ]));
                  Object.assign(novos, extrair(s["RespiratÃ³rio"]||"", [
                    ["po2",  re(`pO2[:\\s]+${NUM}`)],
                    ["pco2", re(`pCO2[:\\s]+${NUM}`)],
                  ]));
                  ...
                  // Extras com categoria selecionada â tambÃ©m vai para a tabela
                  const EXTRAS_PARA_KEY = {
                    'hemoglobina':'hb','hematÃ³crito':'ht','hematocrito':'ht',
                    'leucÃ³cito':'leuco','leucocito':'leuco',
                    'neutrÃ³filo':'neut','neutrofilo':'neut',
                    'bastÃ£o':'bast','bastao':'bast','bastonete':'bast',
                    'linfÃ³cito':'linf','linfocito':'linf',
                    'plaqueta':'plaq',
                    'rni':'rni','inr':'rni','fibrinogÃªnio':'fibri','fibrinogenio':'fibri','ttpa':'ttpa',
                    'creatinina':'cr','ureia':'ur','urÃ©ia':'ur',
                    'sÃ³dio':'na','sodio':'na','potÃ¡ssio':'k','potassio':'k',
                    'magnÃ©sio':'mg','magnesio':'mg',
                    'cÃ¡lcio':'cai','calcio':'cai',
                    'fÃ³sforo':'p','fosforo':'p',
                    ...
                    'diurese':'diur','balanÃ§o':'bh','balanco':'bh',
                  };
                  (d.extras||[]).forEach(ex=>{
                    const cat = ex.categoria || ex.sugestao;
                    if (!cat) return; // sÃ³ lanÃ§a se categoria foi selecionada
                    const nl = (ex.nome||'').toLowerCase();
                    const numMatch = (ex.valor||'').match(/([0-9]+[.,][0-9]+|[0-9]+)/);
                    if (!numMatch) return;
                    const numVal = numMatch[1].replace(',','.');
                    // Tenta achar key padrÃ£o
                    let achou = false;
                    for (const [k, tkey] of Object.entries(EXTRAS_PARA_KEY)) {
                      if (nl.includes(k)) { novos[tkey] = numVal; achou = true; break; }
                    }
                    // Se nÃ£o achou key padrÃ£o, usa o nome do exame como key dinÃ¢mica
                    if (!achou) {
                      const keyDinamica = `_extra_${ex.nome.toLowerCase().replace(/\s+/g,'_').replace(/[^a-z0-9_]/g,'')}`;
                      novos[keyDinamica] = numVal; // salva sÃ³ o valor numÃ©rico
                    }
                  });

                  setTabelaData(t=>{
                    // Merge labs extraÃ­dos via regex + controles extraÃ­dos direto pela IA
              ...
              !leito.paciente ? (
                <div style={{textAlign:"center",padding:60,color:"#334155"}}>
                  ...
                  <div>Cadastre o paciente primeiro na aba <strong style={{color:"#38bdf8"}}>Paciente & CÃ¡lculos</strong></div>
                </div>
              ) : (
                <div style={{width:"100%",maxWidth:contentMedium}}>
                  {dadosIA&&<div style={{background:"rgba(56,189,248,0.07)",border:"1px solid rgba(56,189,248,0.2)",borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:13,color:"#86efac"}}>â Dados da IA aplicados â revise e edite abaixo</div>}
              ...
              <div style={{width:"100%",maxWidth:contentMedium}}>
                <div style={{marginBottom:20}}>
                  <div style={{fontSize:15,fontWeight:700,marginBottom:6,color:T.text1}}>Metas do plantÃ£o</div>
                  <div style={{fontSize:13,color:T.text3}}>Adicione metas e acompanhe o cumprimento durante o plantÃ£o.</div>
                </div>
                <MetasPanel
