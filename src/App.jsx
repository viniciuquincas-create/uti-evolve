...
const ANALGESIA_CATALOGO=[
  {grupo:"Simples",itens:[{id:"dipirona",nome:"Dipirona",doses:["1g","2g"],intervalos:["q6h","q4h"],doseComUnidade:true},{id:"paracetamol",nome:"Paracetamol",doses:["500","750","1000"],intervalos:["6/6h"]}]},
  {grupo:"Opióides",itens:[{id:"tramadol",nome:"Tramadol",doses:["50","100"]},{id:"morfina",nome:"Morfina",doses:["2","4","5","10"]},{id:"metadona",nome:"Metadona",doses:["5","10"]}]},
  {grupo:"Gabapentinoides",itens:[{id:"gabapentina",nome:"Gabapentina",doses:["100","300","400","600"]},{id:"pregabalina",nome:"Pregabalina",doses:["25","50","75","150"]}]},
  {grupo:"Tricíclicos",itens:[{id:"amitriptilina",nome:"Amitriptilina",doses:["10","25","50"]}]},
  {grupo:"Duais",itens:[{id:"duloxetina",nome:"Duloxetina",doses:["30","60"]}]},
];
const ANALGESIA_INTERVALOS=["q4h","q6h","q8h","q12h","q24h","SOS"];
const normalizarIntervalo=v=>{const t=String(v||"").trim();return /^\d+$/.test(t)?`q${t}h`:t;};
...
const formatarAnalgesiaItens=itens=>(Array.isArray(itens)?itens:[]).map(item=>{
  ...
  if(item.esquema)return `${nome} ${item.esquema}`;
...
const LAXATIVO_CATALOGO=[
  {id:"lactulose",nome:"Lactulose",doses:["10 mL","20 mL","30 mL"],intervalos:["q8h","q12h","q24h"],doseComUnidade:true},
  {id:"macrogol",nome:"Macrogol",doses:["1 sachê","2 sachês"],intervalos:["q12h","q24h"],doseComUnidade:true},
  {id:"bisacodil",nome:"Bisacodil",doses:["5","10"],intervalos:["q24h","SOS"]},
  {id:"enema",nome:"Enema",doses:["1 unidade"],intervalos:["q24h","SOS"],doseComUnidade:true},
];
const formatarLaxativos=itens=>(Array.isArray(itens)?itens:[]).map(item=>{const d=LAXATIVO_CATALOGO.find(x=>x.id===item.id);return [d?.nome||item.nome||item.id,item.dose?(d?.doseComUnidade?item.dose:`${item.dose} mg`):"",item.intervalo||""].filter(Boolean).join(" ")}).join(" · ");
...
function MedicacoesTgiEstruturadas({value,onChange,freeValue="",onFreeChange,catalog=PROCINETICO_CATALOGO,formatter=formatarProcineticos,placeholder="Sem pró-cinético ou digite…",idPrefix="proc"}){
  ...
  const upd=(id,patch)=>commit(itens.map(x=>x.id===id?{...x,...patch}:x));
  const remove=id=>{commit(itens.filter(x=>x.id!==id));if(editingId===id)setEditingId("");};
  const editarTexto=novo=>{const norm=novo.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),restantes=itens.filter(item=>norm.includes((catalog.find(x=>x.id===item.id)?.nome||item.id).normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase()));if(restantes.length!==itens.length)onChange(restantes);onFreeChange?.(novo);};
  ...
  return <div style={{position:"relative"}}>
    <input ref={inputRef} value={freeValue||""} onChange={e=>editarTexto(e.target.value)} onFocus={()=>setFocused(true)} onBlur={()=>setTimeout(()=>setFocused(false),160)} onKeyDown={e=>{if(!sugestoes.length||open)return;if(e.key==="ArrowDown"){e.preventDefault();setIdx(i=>(i+1)%sugestoes.length);}else if(e.key==="ArrowUp"){e.preventDefault();setIdx(i=>(i-1+sugestoes.length)%sugestoes.length);}else if(e.key==="Enter"){e.preventDefault();add(sugestoes[idx]?.id,true);}}} placeholder={placeholder} style={{width:"100%",boxSizing:"border-box",height:36,background:T.bgInput,border:`1px solid ${T.borderStrong}`,borderRadius:8,padding:"6px 42px 6px 10px",color:T.text1,fontSize:11}}/>
    <button type="button" aria-label={open?"Minimizar menu":"Maximizar menu"} onMouseDown={e=>e.preventDefault()} onClick={()=>setOpen(v=>!v)} style={{position:"absolute",right:0,top:0,width:36,height:36,borderRadius:"0 8px 8px 0",border:`1px solid ${open?T.accentBorder:T.border}`,background:open?T.accentBg:T.bgCard,color:open?T.accent:T.text3,fontSize:13,fontWeight:700,cursor:"pointer"}}>{open?"▲":"▼"}</button>
    ...
    {open&&<div style={{position:"absolute",zIndex:46,left:0,right:0,top:39,padding:8,display:"grid",gap:6,border:`1px solid ${T.accentBorder}`,borderRadius:8,background:T.bgCard,boxShadow:T.shadowCard,minWidth:310}}>
      <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{catalog.map(m=>{const ativo=itens.some(x=>x.id===m.id);return <button key={m.id} type="button" onClick={()=>ativo?remove(m.id):add(m.id)} style={{padding:"3px 7px",borderRadius:11,border:`1px solid ${ativo?T.accentBorder:T.border}`,background:ativo?T.accentBg:T.bgCard,color:ativo?T.accent:T.text3,fontSize:8,fontWeight:700,cursor:"pointer"}}>{ativo?"✓ ":"+ "}{m.nome}</button>})}</div>
      {itens.map(item=>{const def=catalog.find(x=>x.id===item.id);return <div key={item.id} style={{display:"grid",gridTemplateColumns:"90px 1fr 1fr 28px",gap:5,alignItems:"end",padding:6,border:`1px solid ${T.border}`,borderRadius:7}}><b style={{fontSize:9,alignSelf:"center"}}>{def?.nome||item.id}</b><label style={{fontSize:7,color:T.text4}}>DOSE {def?.doseComUnidade?"":"(mg)"}<input ref={el=>{if(el)doseRefs.current[item.id]=el}} list={`${idPrefix}-dose-${item.id}`} value={item.dose||""} onChange={e=>upd(item.id,{dose:e.target.value})} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();intervalRefs.current[item.id]?.focus()}}} style={{display:"block",width:"100%",boxSizing:"border-box",height:28,marginTop:2,background:T.bgInput,border:`1px solid ${T.border}`,borderRadius:5,color:T.text1,padding:"0 6px"}}/><datalist id={`${idPrefix}-dose-${item.id}`}>{(def?.doses||[]).map(x=><option key={x} value={x}/>)}</datalist></label><label style={{fontSize:7,color:T.text4}}>INTERVALO<input ref={el=>{if(el)intervalRefs.current[item.id]=el}} list={`${idPrefix}-int-${item.id}`} value={item.intervalo||""} onChange={e=>upd(item.id,{intervalo:e.target.value})} onBlur={e=>{const v=normalizarIntervalo(e.target.value);if(v!==e.target.value)upd(item.id,{intervalo:v})}} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();const v=normalizarIntervalo(e.currentTarget.value);if(v!==e.currentTarget.value)upd(item.id,{intervalo:v});voltar()}}} style={{display:"block",boxSizing:"border-box",width:"100%",height:28,marginTop:2,background:T.bgInput,border:`1px solid ${T.border}`,borderRadius:5,color:T.text1,padding:"0 5px"}}/><datalist id={`${idPrefix}-int-${item.id}`}>{(def?.intervalos||[]).map(x=><option key={x} value={x}/>)}</datalist></label><button type="button" onClick={()=>remove(item.id)} style={{height:28,borderRadius:5,border:"1px solid rgba(248,113,113,.25)",background:"rgba(248,113,113,.06)",color:"#f87171"}}>✕</button></div>})}
    </div>}
  </div>;
}
...
const DIAGNOSTICOS_PROBLEMAS_PRESETS=[
  ...
  {nome:"Infarto agudo do miocárdio",campos:[
    ...
    {key:"heart",label:"HEART",tipo:"calculator"},
    {key:"killip",label:"Killip",tipo:"select",opcoes:["I — sem sinais de insuficiência cardíaca","II — estertores <50%, B3 ou congestão venosa","III — edema agudo de pulmão","IV — choque cardiogênico"]},
    {key:"grace",label:"GRACE",tipo:"calculator"},
  ]},
  {nome:"TCE",aliases:["Traumatismo cranioencefálico"],campos:[
    {key:"marshall",label:"Marshall (TC)",tipo:"select",opcoes:["I — lesão difusa, sem patologia visível","II — cisternas presentes, desvio 0–5 mm, sem lesão >25 cm³","III — cisternas comprimidas/ausentes, desvio 0–5 mm, sem lesão >25 cm³","IV — desvio da linha média >5 mm, sem lesão >25 cm³","V — lesão de massa evacuada","VI — lesão de massa não evacuada >25 cm³"]},
  ...
  {nome:"AVCi",aliases:["Acidente vascular cerebral isquêmico"],campos:[{key:"nihss",label:"NIHSS",tipo:"calculator"}]},
  {nome:"AVCh",aliases:["Acidente vascular cerebral hemorrágico","Hemorragia intracerebral"],campos:[{key:"ich",label:"ICH Score",tipo:"calculator"}]},
  {nome:"TEP",aliases:["Tromboembolismo pulmonar"],campos:[{key:"spesi",label:"sPESI",tipo:"calculator"},{key:"pesi",label:"PESI",tipo:"calculator"}]},
  {nome:"Pneumonia adquirida na comunidade",campos:[{key:"curb65",label:"CURB-65",tipo:"calculator"}]},
  {nome:"Pancreatite aguda",campos:[{key:"bisap",label:"BISAP",tipo:"calculator"}]},
  {nome:"Cirrose / insuficiência hepática",campos:[{key:"childPugh",label:"Child-Pugh",tipo:"calculator"},{key:"meldNa",label:"MELD-Na",tipo:"calculator"}]},
...
const subitensDiagnosticoProblema=item=>{
  ...
  const scores=[...(preset?.campos||[]).map(c=>{
...
const textoDiagnosticoProblema=item=>{
  ...
  return `${nomeDiagnosticoProblema(item)}${subitens.length?`\n${subitens.map(s=>`   • ${s}`).join("\n")}`:""}`;
};
const gracePontos=(v,faixas)=>{const n=Number(v);if(!Number.isFinite(n))return null;return faixas.find(([max])=>n<max)?.[1]??faixas.at(-1)[1];};
const calcularGrace=params=>{
  const idade=gracePontos(params.idade,[[30,0],[40,8],[50,25],[60,41],[70,58],[80,75],[90,91],[Infinity,100]]);
  const fc=gracePontos(params.fc,[[50,0],[70,3],[90,9],[110,15],[150,24],[200,38],[Infinity,46]]);
  const pas=gracePontos(params.pas,[[80,58],[100,53],[120,43],[140,34],[160,24],[200,10],[Infinity,0]]);
  const cr=gracePontos(params.creatinina,[[.4,1],[.8,4],[1.2,7],[1.6,10],[2,13],[4,21],[Infinity,28]]);
  if([idade,fc,pas,cr].some(x=>x===null)||!params.killip)return null;
  return idade+fc+pas+cr+({I:0,II:20,III:39,IV:59}[params.killip]||0)+(params.parada?39:0)+(params.desvioST?28:0)+(params.marcador?14:0);
};
function GraceEditor({item,onUpdate,T}){
  const params=item.campos?.graceParams||{};
  const set=(key,value)=>{const next={...params,[key]:value};onUpdate({campos:{...(item.campos||{}),graceParams:next,grace:calcularGrace(next)??""}});};
  const inputs=[['idade','Idade','anos'],['fc','Frequência cardíaca','bpm'],['pas','Pressão arterial sistólica','mmHg'],['creatinina','Creatinina','mg/dL']];
  return <div style={{marginTop:5,padding:7,borderRadius:7,border:`1px solid ${T.accentBorder}`,background:T.bgInput}} onClick={e=>e.stopPropagation()}>
    <b style={{fontSize:9,color:T.text2}}>GRACE — mortalidade intra-hospitalar</b>
    <div style={{display:'grid',gap:5,marginTop:6}}>{inputs.map(([key,label,unit])=><label key={key} style={{display:'grid',gridTemplateColumns:'1fr 70px',gap:5,alignItems:'center',fontSize:8.5,color:T.text3}}>{label}<span style={{display:'flex'}}><input type="number" step={key==='creatinina'?'.01':'1'} value={params[key]||''} onChange={e=>set(key,e.target.value)} style={{width:'100%',minWidth:0,padding:'4px',border:`1px solid ${T.border}`,background:T.bgCard,color:T.text1,borderRadius:'5px 0 0 5px',fontSize:9}}/><small style={{padding:'5px 3px',border:`1px solid ${T.border}`,borderLeft:0,borderRadius:'0 5px 5px 0',fontSize:7,color:T.text4}}>{unit}</small></span></label>)}</div>
    <label style={{display:'block',fontSize:8.5,color:T.text3,marginTop:5}}>Killip<select value={params.killip||''} onChange={e=>set('killip',e.target.value)} style={{width:'100%',padding:4,borderRadius:5,border:`1px solid ${T.border}`,background:T.bgCard,color:T.text1,fontSize:9}}><option value="">— selecionar —</option>{['I','II','III','IV'].map(x=><option key={x}>{x}</option>)}</select></label>
    {[['parada','Parada cardíaca na admissão'],['desvioST','Desvio do segmento ST'],['marcador','Marcador de necrose elevado']].map(([key,label])=><label key={key} style={{display:'flex',gap:5,alignItems:'center',fontSize:8.5,color:T.text2,marginTop:5}}><input type="checkbox" checked={!!params[key]} onChange={e=>set(key,e.target.checked)}/>{label}</label>)}
    <div style={{marginTop:6,padding:5,borderRadius:5,background:T.accentBg,color:T.accent,fontSize:10,fontWeight:800}}>GRACE: {item.campos?.grace!==''&&item.campos?.grace!==undefined?item.campos.grace:'preencha todos os parâmetros'}</div>
  </div>;
}

const opt=(...labels)=>labels.map((label,value)=>({label,value}));
const SCORE_DEFS={
  ich:{title:"ICH Score",fields:[{key:"gcs",label:"Glasgow",options:[{label:"13–15 — 0",value:0},{label:"5–12 — 1",value:1},{label:"3–4 — 2",value:2}]},{key:"idade80",label:"Idade ≥80 anos",type:"bool"},{key:"volume30",label:"Volume do hematoma ≥30 cm³",type:"bool"},{key:"hiv",label:"Hemorragia intraventricular",type:"bool"},{key:"infra",label:"Origem infratentorial",type:"bool"}],interpret:n=>`pontuação ${n} (0–6; maior pontuação = maior risco)`},
  heart:{title:"HEART",fields:[
    {key:"historia",label:"História clínica",options:opt("Pouco suspeita — 0","Moderadamente suspeita — 1","Altamente suspeita — 2")},
    {key:"ecg",label:"ECG",options:opt("Normal — 0","Alteração inespecífica da repolarização — 1","Depressão significativa de ST — 2")},
    {key:"idade",label:"Idade",options:opt("<45 anos — 0","45–64 anos — 1","≥65 anos — 2")},
    {key:"riscos",label:"Fatores de risco",options:opt("Nenhum — 0","1–2 fatores — 1","≥3 fatores ou aterosclerose conhecida — 2")},
    {key:"troponina",label:"Troponina",options:opt("≤ limite normal — 0","1–3× o limite — 1",">3× o limite — 2")},
  ],interpret:n=>n<=3?"baixo risco":n<=6?"risco intermediário":"alto risco"},
  rotterdam:{title:"Rotterdam (TC)",offset:1,fields:[
    {key:"cisternas",label:"Cisternas basais",options:opt("Normais — 0","Comprimidas — 1","Ausentes — 2")},
    {key:"desvio",label:"Desvio da linha média",options:opt("≤5 mm — 0",">5 mm — 1")},
    {key:"epidural",label:"Lesão epidural",options:[{label:"Presente — 0",value:0},{label:"Ausente — 1",value:1}]},
    {key:"hivHsa",label:"HIV ou HSA traumática",options:opt("Ausente — 0","Presente — 1")},
  ],interpret:n=>`classe ${n} (1–6; maior pontuação = pior prognóstico)`},
  spesi:{title:"sPESI",fields:[
    ...["Idade >80 anos","Câncer","Insuficiência cardíaca ou pulmonar crônica","FC ≥110 bpm","PAS <100 mmHg","SatO₂ <90%"].map((label,i)=>({key:`f${i}`,label,type:"bool"}))
  ],interpret:n=>n===0?"baixo risco":"alto risco (≥1)"},
  pesi:{title:"PESI",fields:[
    {key:"idade",label:"Idade (anos)",type:"number",points:v=>Number(v)||0},
    {key:"masculino",label:"Sexo masculino",type:"bool",points:10},{key:"cancer",label:"Câncer",type:"bool",points:30},{key:"ic",label:"Insuficiência cardíaca",type:"bool",points:10},{key:"dpoc",label:"Doença pulmonar crônica",type:"bool",points:10},
    {key:"fc",label:"FC ≥110 bpm",type:"bool",points:20},{key:"pas",label:"PAS <100 mmHg",type:"bool",points:30},{key:"fr",label:"FR ≥30 irpm",type:"bool",points:20},{key:"temp",label:"Temperatura <36 °C",type:"bool",points:20},{key:"mental",label:"Alteração do estado mental",type:"bool",points:60},{key:"sat",label:"SatO₂ <90%",type:"bool",points:20},
  ],interpret:n=>n<=65?"classe I":n<=85?"classe II":n<=105?"classe III":n<=125?"classe IV":"classe V"},
  curb65:{title:"CURB-65",fields:[...['Confusão','Ureia >50 mg/dL (ou >7 mmol/L)','FR ≥30 irpm','PAS <90 ou PAD ≤60 mmHg','Idade ≥65 anos'].map((label,i)=>({key:`f${i}`,label,type:'bool'}))],interpret:n=>n<=1?"baixo risco":n===2?"risco intermediário":"alto risco"},
  bisap:{title:"BISAP",fields:[...['Ureia >25 mg/dL','Alteração do estado mental','SIRS','Idade >60 anos','Derrame pleural'].map((label,i)=>({key:`f${i}`,label,type:'bool'}))],interpret:n=>n<3?"menor risco":"maior risco (≥3)"},
  childPugh:{title:"Child-Pugh",fields:[
    {key:'bilirrubina',label:'Bilirrubina',options:opt('<2 mg/dL — 1','2–3 mg/dL — 2','>3 mg/dL — 3')},{key:'albumina',label:'Albumina',options:opt('>3,5 g/dL — 1','2,8–3,5 g/dL — 2','<2,8 g/dL — 3')},{key:'inr',label:'INR',options:opt('<1,7 — 1','1,7–2,3 — 2','>2,3 — 3').map(x=>({...x,value:x.value+1}))},{key:'ascite',label:'Ascite',options:opt('Ausente — 1','Leve/controlada — 2','Moderada-grave/refratária — 3').map(x=>({...x,value:x.value+1}))},{key:'encefalopatia',label:'Encefalopatia',options:opt('Ausente — 1','Grau I–II — 2','Grau III–IV — 3').map(x=>({...x,value:x.value+1}))},
  ],offset:2,interpret:n=>n<=6?"classe A":n<=9?"classe B":"classe C"},
  meldNa:{title:"MELD-Na",fields:[{key:'bilirrubina',label:'Bilirrubina (mg/dL)',type:'number'},{key:'inr',label:'INR',type:'number'},{key:'creatinina',label:'Creatinina (mg/dL)',type:'number'},{key:'sodio',label:'Sódio (mEq/L)',type:'number'},{key:'dialise',label:'Diálise ≥2 vezes na última semana',type:'bool'}],custom:p=>{if([p.bilirrubina,p.inr,p.creatinina,p.sodio].some(v=>v===''||v==null))return null;let b=Math.max(1,+p.bilirrubina),i=Math.max(1,+p.inr),c=p.dialise?4:Math.min(4,Math.max(1,+p.creatinina)),na=Math.min(137,Math.max(125,+p.sodio));if(![b,i,c,na].every(Number.isFinite))return null;let meld=10*(.957*Math.log(c)+.378*Math.log(b)+1.12*Math.log(i)+.643);meld=Math.round(Math.min(40,Math.max(6,meld)));return Math.round(Math.min(40,Math.max(6,meld+1.32*(137-na)-.033*meld*(137-na))));},interpret:n=>`pontuação ${n} (6–40)`},
  nihss:{title:"NIHSS",fields:[
    ['consc','1a. Nível de consciência',opt('Alerta — 0','Sonolento — 1','Estupor — 2','Coma — 3')],['perg','1b. Perguntas',opt('Ambas corretas — 0','Uma correta — 1','Nenhuma — 2')],['cmd','1c. Comandos',opt('Ambos corretos — 0','Um correto — 1','Nenhum — 2')],['olhar','2. Olhar conjugado',opt('Normal — 0','Paresia parcial — 1','Desvio forçado — 2')],['visual','3. Campos visuais',opt('Normal — 0','Hemianopsia parcial — 1','Hemianopsia completa — 2','Cegueira bilateral — 3')],['face','4. Paralisia facial',opt('Normal — 0','Leve — 1','Parcial — 2','Completa — 3')],['bracoE','5a. Braço esquerdo',opt('Sem queda — 0','Queda — 1','Algum esforço contra gravidade — 2','Sem esforço contra gravidade — 3','Sem movimento — 4')],['bracoD','5b. Braço direito',opt('Sem queda — 0','Queda — 1','Algum esforço contra gravidade — 2','Sem esforço contra gravidade — 3','Sem movimento — 4')],['pernaE','6a. Perna esquerda',opt('Sem queda — 0','Queda — 1','Algum esforço contra gravidade — 2','Sem esforço contra gravidade — 3','Sem movimento — 4')],['pernaD','6b. Perna direita',opt('Sem queda — 0','Queda — 1','Algum esforço contra gravidade — 2','Sem esforço contra gravidade — 3','Sem movimento — 4')],['ataxia','7. Ataxia',opt('Ausente — 0','Um membro — 1','Dois membros — 2')],['sens','8. Sensibilidade',opt('Normal — 0','Perda leve-moderada — 1','Perda grave — 2')],['ling','9. Linguagem',opt('Normal — 0','Afasia leve-moderada — 1','Afasia grave — 2','Mudo/afasia global — 3')],['dis','10. Disartria',opt('Normal — 0','Leve-moderada — 1','Grave — 2')],['ext','11. Extinção/desatenção',opt('Ausente — 0','Uma modalidade — 1','Profunda/mais de uma modalidade — 2')]
  ].map(([key,label,options])=>({key,label,options})),interpret:n=>n===0?'sem déficit':n<=4?'déficit leve':n<=15?'moderado':n<=20?'moderado-grave':'grave'},
};
const calcularScoreGenerico=(def,p)=>{if(def.custom)return def.custom(p);let total=def.offset||0;for(const f of def.fields){if(f.type==='number'){if(p[f.key]===''||p[f.key]==null)return null;total+=f.points?f.points(p[f.key]):0;}else if(f.type==='bool'){if(p[f.key])total+=typeof f.points==='number'?f.points:1;}else{if(p[f.key]==null||p[f.key]==='')return null;total+=Number(p[f.key]);}}return total;};
function ScoreEditor({scoreKey,item,onUpdate,T}){
 const def=SCORE_DEFS[scoreKey];if(!def)return null;const pk=`${scoreKey}Params`,p=item.campos?.[pk]||{};
 const set=(key,value)=>{const next={...p,[key]:value},result=calcularScoreGenerico(def,next);onUpdate({campos:{...(item.campos||{}),[pk]:next,[scoreKey]:result??''}});};
 const result=item.campos?.[scoreKey];
 return <div style={{marginTop:5,padding:7,borderRadius:7,border:`1px solid ${T.accentBorder}`,background:T.bgInput}} onClick={e=>e.stopPropagation()}><b style={{fontSize:9,color:T.text2}}>{def.title}</b><div style={{display:'grid',gap:5,marginTop:6}}>{def.fields.map(f=>f.type==='bool'?<label key={f.key} style={{display:'flex',gap:5,fontSize:8.5,color:T.text2}}><input type="checkbox" checked={!!p[f.key]} onChange={e=>set(f.key,e.target.checked)}/>{f.label}</label>:f.type==='number'?<label key={f.key} style={{fontSize:8.5,color:T.text3}}>{f.label}<input type="number" step="any" value={p[f.key]||''} onChange={e=>set(f.key,e.target.value)} style={{display:'block',width:'100%',boxSizing:'border-box',padding:4,borderRadius:5,border:`1px solid ${T.border}`,background:T.bgCard,color:T.text1,fontSize:9}}/></label>:<label key={f.key} style={{fontSize:8.5,color:T.text3}}>{f.label}<select value={p[f.key]??''} onChange={e=>set(f.key,e.target.value===''?'':Number(e.target.value))} style={{display:'block',width:'100%',padding:4,borderRadius:5,border:`1px solid ${T.border}`,background:T.bgCard,color:T.text1,fontSize:9}}><option value="">— selecionar —</option>{f.options.map(o=><option key={o.label} value={o.value}>{o.label}</option>)}</select></label>)}</div><div style={{marginTop:6,padding:5,borderRadius:5,background:T.accentBg,color:T.accent,fontSize:10,fontWeight:800}}>{def.title}: {result!==''&&result!==undefined?`${result} — ${def.interpret(result)}`:'preencha os parâmetros'}</div></div>;
}
        ...
        <div style={{background:T.bgCard,border:"1px solid rgba(239,68,68,0.32)",
          ...
          {!!diagnosticosProblemas.length&&<div style={{display:"grid",gap:6,marginBottom:8}}>{diagnosticosProblemas.map(item=>{const preset=presetDiagnosticoProblema(item.nome),expanded=diagnosticoAberto===item.id;return <div key={item.id} data-diagnostico-card={item.id} tabIndex={0} onContextMenu={e=>{e.preventDefault();e.stopPropagation();setMenuDiagnostico({id:item.id,x:e.clientX,y:e.clientY});}} onKeyDown={e=>{if(e.target!==e.currentTarget)return;if(e.key==="ContextMenu"||(e.shiftKey&&e.key==="F10")){e.preventDefault();const r=e.currentTarget.getBoundingClientRect();setMenuDiagnostico({id:item.id,x:r.left,y:r.top+20});}}} onFocus={e=>{if(!e.target.closest("[data-toggle-diagnostico]"))setDiagnosticoAberto(item.id);}} onMouseDown={e=>{if(!e.target.closest("[data-toggle-diagnostico]" )&&!e.target.closest("button[data-remove-diagnostico]")&&!e.target.closest("[data-drag-diagnostico]"))setDiagnosticoAberto(item.id)}} onBlur={e=>{if(!e.currentTarget.contains(e.relatedTarget))setDiagnosticoAberto(null)}} onDragOver={e=>{if(diagnosticoArrastando&&diagnosticoArrastando!==item.id)e.preventDefault();}} onDrop={e=>{e.preventDefault();if(!diagnosticoArrastando||diagnosticoArrastando===item.id)return;const lista=[...diagnosticosProblemas],de=lista.findIndex(x=>x.id===diagnosticoArrastando),para=lista.findIndex(x=>x.id===item.id);if(de<0||para<0)return;const [movido]=lista.splice(de,1);lista.splice(para,0,movido);salvarDiagnosticosProblemas(lista);setDiagnosticoArrastando(null);}} style={{padding:"7px",borderRadius:8,border:"1px solid rgba(248,113,113,.3)",background:"rgba(248,113,113,.06)",opacity:diagnosticoArrastando===item.id?.55:1}}>
            ...
            {(item.subitens||[]).map(sub=><div key={sub.id} style={{display:"flex",gap:4,alignItems:"flex-start",marginTop:5}}>
              <span style={{color:T.text3,fontSize:11,paddingTop:4}}>•</span>
              <textarea aria-label="Texto do subitem" value={sub.texto||""} rows={2} onChange={e=>atualizarDiagnostico(item.id,{subitens:item.subitens.map(x=>x.id===sub.id?{...x,texto:e.target.value}:x)})} placeholder="Adicionar informação sobre o diagnóstico…" style={{minWidth:0,flex:1,resize:"vertical",padding:"4px 5px",borderRadius:5,border:`1px solid ${T.border}`,background:T.bgInput,color:T.text1,fontFamily:"inherit",fontSize:9}}/>
              <button title="Remover subitem" onClick={()=>atualizarDiagnostico(item.id,{subitens:item.subitens.filter(x=>x.id!==sub.id)})} style={{border:0,background:"transparent",color:T.text4,cursor:"pointer",padding:"4px 0"}}>×</button>
            </div>)}
...
function SerialMeasurements({title,fieldKey,fields=[],suggestedParams=[],value,onChange,color="#38bdf8",subjective=false,calculator,patient,workflow=false,compactHistory=false,latestOnly=false}){
  ...
  const addSuggested=p=>{
    const grupo=Array.isArray(p.fields)?p.fields:[];
    if(grupo.length){
      const keys=new Set(grupo.map(f=>f.key)),groups=state.paramGroups&&typeof state.paramGroups==="object"?state.paramGroups:{};
      const active=!!groups[p.key];
      if(active){
        const preserve=new Set(groups[p.key]?.preserveKeys||[]),removeKeys=new Set([...keys].filter(k=>!preserve.has(k)));
        const nextGroups={...groups};delete nextGroups[p.key];
        emit({paramGroups:nextGroups,customParams:customParams.filter(f=>!removeKeys.has(f.key)),entries:entries.map(e=>{const values={...(e.values||{})};removeKeys.forEach(k=>delete values[k]);return {...e,values};})});
      }else{
        const preserveKeys=grupo.filter(f=>customParams.some(c=>c.key===f.key)).map(f=>f.key);
        emit({paramGroups:{...groups,[p.key]:{preserveKeys}},customParams:[...customParams,...grupo.filter(f=>!customParams.some(c=>c.key===f.key))]});
      }
      return;
    }
    if(fields.some(f=>f.key===p.key))return;
    const active=customParams.some(f=>f.key===p.key);
    if(active)emit({customParams:customParams.filter(f=>f.key!==p.key),entries:entries.map(e=>{const values={...(e.values||{})};delete values[p.key];return {...e,values};})});
    else emit({customParams:[...customParams,p]});
  };
  return <div style={{marginTop:9,border:`1px solid ${color}33`,borderRadius:9,background:`${color}08`,overflow:"hidden"}}>
    ...
    {entries.map((e,i)=>{const anterior=i>0?entries[i-1]:null;const historica=(latestOnly&&e.id!==latestEntryId)||(compactHistory&&e.data&&e.data!==today);if(historica&&!expandedHistory[e.id]){const preenchidas=Object.values(e.values||{}).filter(v=>String(v??"").trim()).length;return <button key={e.id} onClick={()=>setExpandedHistory(x=>({...x,[e.id]:true}))} style={{width:"100%",display:"flex",alignItems:"center",gap:8,padding:"7px 9px",border:0,borderTop:i?`1px solid ${T.border}`:0,background:T.bgCard,color:T.text2,cursor:"pointer",textAlign:"left"}}><span style={{fontSize:10,fontFamily:mono,color}}>{e.data?new Date(`${e.data}T00:00:00`).toLocaleDateString("pt-BR"):"sem data"}{e.hora?` · ${e.hora}`:""}</span><span style={{fontSize:9,color:T.text3}}>{preenchidas} parâmetro(s){e.interpretacao?" · com interpretação":""}{e.conduta?" · com conduta":""}</span><span style={{marginLeft:"auto",fontSize:10,color:T.text3}}>▾ consultar</span></button>}return <div key={e.id} style={{padding:"8px 9px",borderTop:i?`1px solid ${T.border}`:0}}>
      ...
      {calculator==="swan"&&(()=>{const paps=numPocus(e.values?.paps),papd=numPocus(e.values?.papd),rap=numPocus(e.values?.pvc),papi=paps!==null&&papd!==null&&rap!==null&&rap>0?(paps-papd)/rap:null;return <div style={{marginTop:7,padding:"7px 9px",borderRadius:7,border:`1px solid ${papi!==null?"rgba(52,211,153,.28)":T.border}`,background:papi!==null?"rgba(52,211,153,.07)":T.bgCard,fontSize:10,color:papi!==null?"#34d399":T.text4,fontFamily:mono}}>{papi!==null?<>PAPi <b>{papi.toFixed(2)}</b><span style={{display:"block",marginTop:3,color:T.text3,fontSize:9}}>(PAPs − PAPd) / pressão atrial direita (PVC)</span></>:rap===0?"PAPi não calculável com PVC igual a zero.":"Preencha PAPs, PAPd e PVC para calcular a PAPi."}</div>})()}
    </div>})}
...
const SERIAL_MONITOR_CONFIG={
  ...
  cvPocusSerial:{title:"POCUS — CICLO HEMODINÂMICO",color:"#f87171",subjective:true,calculator:"pocus-co",workflow:true,compactHistory:true,fields:[{key:"vciMax",label:"VCI maior (cm)",reference:"≤2,1"},{key:"vciMin",label:"VCI menor (cm)"},{key:"lvotDiam",label:"Diâmetro VSVE (cm)"},{key:"vtiVE",label:"VTI VE (cm)",reference:"18–22"},{key:"fc",label:"FC (bpm)"},{key:"vtiVD",label:"VTI VD (cm)",reference:">18"}],suggestedParams:[{key:"ea",label:"E/A",reference:"0,8–2,0"},{key:"ee",label:"E/e′",reference:"<14 (média); septal <15"},{key:"eprime",label:"e′ (cm/s)",reference:"septal ≥7; lateral ≥10"},{key:"feve",label:"FEVE (%)"},{key:"__vac",label:"VAC",help:"Adiciona FEVE, PAS, PAD, PAM, tNd e VSF para Ea, Ees e VAC",fields:[{key:"feve",label:"FEVE (%)"},{key:"pas",label:"PAS (mmHg)"},{key:"pad",label:"PAD (mmHg)"},{key:"pam",label:"PAM (mmHg)"},{key:"tnd",label:"tNd",reference:"razão 0–1"},{key:"vsf",label:"VSF (mL)"}]},{key:"mapse",label:"MAPSE (mm)",reference:">10"},{key:"epss",label:"EPSS (mm)",reference:"≤7"},{key:"tacc",label:"TAcc (ms)",reference:">130"},{key:"fac",label:"FAC (%)",reference:"≥35%"},{key:"tapse",label:"TAPSE (mm)",reference:"≥17"},{key:"psap",label:"PSAP (mmHg)",reference:"≤35"}]},
...
function EvolucaoEditor({ leito, campos, onCampoEdit, config={}, tabelaHoje={}, tabelaDataLeito={}, onMetaChange, metas=[], onLeitoChange }) {
  ...
  const txtCv = () => {
    ...
    {const perf=[get("cvExtremidades")?`Extremidades ${get("cvExtremidades").toLowerCase()}`:"",get("cvPerf")].filter(Boolean).join(" · ");if(perf)p.push(`- Perfusão: ${perf}`);}
  ...
  const serialLines=(fieldKey,label)=>{
    ...
    return entradas.map(entry=>{
      ...
      const swanPapi=fieldKey==="cvSwanSerial"&&numPocus(entry.values?.paps)!==null&&numPocus(entry.values?.papd)!==null&&numPocus(entry.values?.pvc)>0?(numPocus(entry.values.paps)-numPocus(entry.values.papd))/numPocus(entry.values.pvc):null;
      ...
      const values=[...Object.entries(entry.values||{}).filter(([,v])=>String(v??"").trim()).map(([key,v])=>`${labels[key]||key} ${String(v).trim()}`),...calculated,...(swanPapi!==null?[`PAPi ${swanPapi.toFixed(2)}`]:[])].join(" / ");
  ...
  const txtCvFull = () => {
    ...
    if(get("cvDVA"))  p.push(`- DVA: ${get("cvDVA")}`);
    if(get("cv24h"))  p.push(`- 24h: ${get("cv24h")}`);
    {const pf=[get("cvExtremidades")?`Extremidades ${get("cvExtremidades").toLowerCase()}`:null,get("cvTEC")?`TEC ${get("cvTEC")}`:null,get("cvLact")?`Lactato ${get("cvLact")} mmol/L`:null,vis["cvDeltaCO2"]&&get("cvDeltaCO2")?`ΔCO₂ ${get("cvDeltaCO2")} mmHg`:null,vis["cvDeltaCO2"]&&get("cvDeltaPP")?`ΔPP ${get("cvDeltaPP")}%`:null].filter(Boolean).join(" · ");if(pf)p.push(`- Perfusão: ${pf}`);}
    if(vis.cvTropo){
      const allTropos = (()=>{
        try {
    ...
    <div>
      ...
      <SysB id="tgi" sigla="== TGI:" label="Gastrointestinal" color={"#fb923c"} txtFn={txtTGIFull}
        ...
        <Row>
          ...
          <Col><FL>Laxativos</FL><MedicacoesTgiEstruturadas value={campos.tgLaxativosItens} onChange={v=>onCampoEdit("tgLaxativosItens",v)} freeValue={campos.tgLaxativos} onFreeChange={v=>onCampoEdit("tgLaxativos",v)} catalog={LAXATIVO_CATALOGO} formatter={formatarLaxativos} placeholder="Sem laxativos ou digite…" idPrefix="lax"/></Col>
          <Col><FL>Pró-cinéticos</FL><MedicacoesTgiEstruturadas value={campos.tgProcineticosItens} onChange={v=>onCampoEdit("tgProcineticosItens",v)} freeValue={campos.tgProcineticos} onFreeChange={v=>onCampoEdit("tgProcineticos",v)}/></Col>
          <Col><FL>Profilaxia LAMG</FL><div style={{position:"relative"}}><input list="tgi-lamg" value={campos.tgLAMG||""} onChange={e=>onCampoEdit("tgLAMG",e.target.value)} placeholder="Sem profilaxia ou esquema…" style={{width:"100%",boxSizing:"border-box",height:36,background:T.bgInput,border:`1px solid ${T.borderStrong}`,borderRadius:8,padding:"6px 42px 6px 10px",color:T.text1,fontSize:12}}/><span aria-hidden="true" style={{position:"absolute",right:0,top:0,width:36,height:36,display:"grid",placeItems:"center",borderLeft:`1px solid ${T.border}`,borderRadius:"0 8px 8px 0",background:T.bgCard,color:T.text3,fontSize:13,pointerEvents:"none"}}>▼</span><datalist id="tgi-lamg">{getHospitalConfig(config).lamgOpcoes.map(x=><option key={x} value={x}/>)}</datalist></div></Col>
        </Row>
<Row>
...
// UTI G4 — dois conjuntos laterais com corredor central e entrada inferior.
// Parede esquerda: 04–06 em cima e 03–01 embaixo. Parede direita:
// 07–09 em cima, 10–11 descendo e 14–12 embaixo.
const G4_LAYOUT={
  "1":{gridColumn:"3 / 4",gridRow:"4"},
  "2":{gridColumn:"2 / 3",gridRow:"4"},
  "3":{gridColumn:"1 / 2",gridRow:"4"},
  "4":{gridColumn:"1 / 2",gridRow:"1"},
  "5":{gridColumn:"2 / 3",gridRow:"1"},
  "6":{gridColumn:"3 / 4",gridRow:"1"},
  "7":{gridColumn:"6 / 7",gridRow:"1"},
  "8":{gridColumn:"7 / 8",gridRow:"1"},
  "9":{gridColumn:"8 / 9",gridRow:"1"},
  "10":{gridColumn:"8 / 9",gridRow:"2"},
  "11":{gridColumn:"8 / 9",gridRow:"3"},
  "12":{gridColumn:"8 / 9",gridRow:"4"},
  "13":{gridColumn:"7 / 8",gridRow:"4"},
  "14":{gridColumn:"6 / 7",gridRow:"4"},
};
...
function ColetaPlantaoPanel({uti,leitos,evolPorLeito,onAplicar}){
  ...
  const importarJson=()=>{setErro("");try{const limpo=String(jsonInput||"").replace(/^\s*```(?:json)?/i,"").replace(/```\s*$/i,"").trim();const parsed=JSON.parse(limpo);if(!Array.isArray(parsed?.leitos))throw new Error("O JSON precisa conter a lista ‘leitos’.");setResultado({data:parsed.data||"",leitos:parsed.leitos});setAplicados(parsed.leitos.map((_,i)=>i));}catch(e){setErro(`JSON inválido: ${e.message||"revise o conteúdo"}`);}};
  ...
  const guiaFolha=(titulo,l)=>({
    ...
    "LAB":<div style={{height:"100%",display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gridTemplateRows:"repeat(5,1fr)",columnGap:7,rowGap:2,alignItems:"center"}}>{[
      ["Hb","hb"],["Le","leuco"],["Plaq","plaq"],["RNI","rni"],
    ...
    "Neuro":<div style={{height:"100%",display:"grid",gridTemplateRows:"repeat(4,1fr)",rowGap:3,alignItems:"center"}}>
    ...
    "CV":<div style={{height:"100%",display:"grid",gridTemplateRows:"repeat(4,1fr)",rowGap:3,alignItems:"center"}}>
  ...
  const camposFolha=[
    ...
    ["Vent","VENTILAÇÃO","13mm"],["Re/Me","RENAL / METABÓLICO","10mm"],["TGI","TGI / NUTRIÇÃO","16mm"],["Infec","INFECCIOSO","16mm"],["Check","PROFILAXIAS","9mm"],
  ];
  return <div style={{height:"100%",overflowY:"auto",padding:"18px 24px",background:T.bgPage}}>
    ...
    <div className="coleta-print">
      {paginas.map((grupo,pagina)=><div className="coleta-page" key={pagina} style={{marginBottom:16,padding:10,border:"1px solid #94a3b8",borderRadius:8,background:"#fff",color:"#0f172a",boxShadow:"0 8px 24px rgba(15,23,42,.08)",overflow:"hidden"}}>
        <div style={{display:"grid",gridTemplateColumns:`92px repeat(${grupo.length},minmax(0,1fr))`,borderTop:"1px solid #475569",borderLeft:"1px solid #475569"}}>
          ...
          {grupo.map(l=><div key={l.id} style={{padding:6,borderRight:"1px solid #475569",borderBottom:"1px solid #475569",height:"22mm"}}><div style={{fontSize:5.8,fontWeight:800,color:"#475569",letterSpacing:.15}}>UNIDADE: {uti?.nome} · UTI-ID: {uti?.id} · LEITO-ID: {l.id}</div><div style={{fontSize:11,fontWeight:850}}>{l.nome} · {l.paciente}</div><div style={{marginTop:2,color:"#64748b",fontSize:6.8,lineHeight:1.25}}><b>Dx:</b> {resumo(l.diagnostico,95)||"—"}<br/>{(l.procedimentos||[]).length>0&&<><b>Proced.:</b> {resumo((l.procedimentos||[]).map(p=>[p.nome,p.data&&`(${dataCurta(p.data)})`].filter(Boolean).join(" ")).join(" · "),95)}<br/></>}<b>Hist.:</b> {resumo([l.doencasPrevias,evolPorLeito?.[l.id]?.hda].filter(Boolean).join(" · "),105)||"—"}</div></div>)}
    ...
    <div className="coleta-tools" style={{marginTop:18,padding:14,border:`1px solid ${T.border}`,borderRadius:12,background:T.bgCard}}>
      ...
      <div style={{marginTop:14,paddingTop:12,borderTop:`1px solid ${T.border}`}}><div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:7}}><b style={{fontSize:11,color:T.text2}}>Importar JSON do ChatGPT</b><button onClick={copiarModeloJson} style={{padding:"4px 8px",borderRadius:6,border:`1px solid ${T.border}`,background:T.bgInput,color:T.text3,cursor:"pointer",fontSize:9}}>Copiar modelo JSON</button></div><textarea value={jsonInput} onChange={e=>setJsonInput(e.target.value)} placeholder='Cole aqui o JSON contendo { "data": "AAAA-MM-DD", "leitos": [...] }' rows={5} style={{width:"100%",resize:"vertical",padding:"8px 10px",borderRadius:8,border:`1px solid ${T.border}`,background:T.bgInput,color:T.text1,fontFamily:mono,fontSize:10}}/><button onClick={importarJson} disabled={!jsonInput.trim()} style={{marginTop:7,padding:"7px 11px",borderRadius:7,border:`1px solid ${T.accentBorder}`,background:T.accentBg,color:T.accent,fontWeight:800,cursor:"pointer",fontSize:10}}>Revisar JSON antes de lançar</button></div>
      {resultado&&<div style={{marginTop:12}}><div style={{fontSize:11,color:T.text2,fontWeight:700,marginBottom:7}}>Reconhecidos: {(resultado.leitos||[]).length} quadro(s){resultado.data?` · ${resultado.data}`:""}</div>{(resultado.leitos||[]).map((r,i)=><label key={i} style={{display:"block",padding:"8px 10px",marginBottom:6,border:`1px solid ${T.border}`,borderRadius:8,background:T.bgInput,color:T.text2,fontSize:10}}><div style={{display:"flex",gap:7,alignItems:"center"}}><input type="checkbox" checked={aplicados.includes(i)} onChange={()=>setAplicados(a=>a.includes(i)?a.filter(x=>x!==i):[...a,i])}/><b>{r.leito||"Leito não identificado"}</b><span>· {r.paciente||"paciente não lido"}</span></div><div style={{marginTop:4,color:T.text3}}>{[...Object.entries(r.labs||{}),...Object.entries(r.controles||{}),...Object.entries(r.bombas||{})].filter(([,v])=>v).map(([k,v])=>`${k} ${v}`).join(" · ")||"Somente anotações clínicas"}</div></label>)}<button onClick={()=>onAplicar({...resultado,leitos:(resultado.leitos||[]).filter((_,i)=>aplicados.includes(i))})} disabled={!aplicados.length} style={{width:"100%",marginTop:4,padding:9,borderRadius:8,border:"none",background:"#0284c7",color:"#fff",fontWeight:850,cursor:"pointer"}}>Confirmar e lançar nos leitos selecionados</button></div>}
    </div>
  </div>;
}
...
function MapaLeitosPanel({uti,hospital,leitos,onAbrirLeito,onVoltar,altaSheetUrl}){
  ...
  const ehG4=ehHsp&&/\bG4\b/i.test(uti?.nome||"");
  const temMapaFisico=ehG1||ehG2||ehG3||ehG4;
  ...
  return <div style={{height:"100%",overflow:"auto",background:T.bgPage,padding:"14px clamp(12px,2vw,28px) 24px",boxSizing:"border-box"}}>
    ...
    <div style={temMapaFisico?{display:"grid",gridTemplateColumns:ehG1?"repeat(9,minmax(0,1fr))":ehG4?"repeat(8,minmax(105px,1fr))":"repeat(4,minmax(120px,170px))",gridTemplateRows:ehG1?"repeat(6,minmax(58px,1fr))":"repeat(4,minmax(88px,118px))",justifyContent:"center",alignContent:"center",gap:ehG1?8:ehG4?10:12,width:ehG1||ehG4?"100%":"min(100%,820px)",minWidth:ehG4?920:undefined,height:ehG1?"calc(100vh - 150px)":ehG4?"min(calc(100vh - 165px),620px)":"min(calc(100vh - 165px),590px)",minHeight:ehG1?430:470,margin:ehG1?0:"14px auto 0",padding:ehG1?12:ehG4?16:22,boxSizing:"border-box",border:`1px solid ${T.border}`,borderRadius:18,background:`radial-gradient(circle at center,${T.accentBg} 0,transparent 42%), ${T.bgCard}`,boxShadow:T.shadowCard}:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12}}>
      ...
      {ehG2&&<><div style={{gridColumn:"2 / 4",gridRow:"2 / 4",display:"flex",flexDirection:"column",gap:8,alignItems:"center",justifyContent:"center",border:`1px dashed ${T.borderStrong}`,borderRadius:22,color:T.textDim,fontFamily:mono,fontSize:10,letterSpacing:2,background:`linear-gradient(145deg,${T.bgInput},${T.bgCard})`}}><span style={{width:36,height:36,display:"grid",placeItems:"center",borderRadius:12,background:T.accentBg,color:T.accent,fontSize:21,fontFamily:"sans-serif",fontWeight:300}}>＋</span><b style={{color:T.text3}}>POSTO CENTRAL</b><small style={{fontSize:7,letterSpacing:1,color:T.text4}}>EQUIPE · MONITORIZAÇÃO</small></div><div style={{gridColumn:"4 / 5",gridRow:"2 / 4",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{padding:"6px 9px",borderRadius:12,border:`1px solid ${T.border}`,background:T.bgInput,color:T.textDim,fontFamily:mono,fontSize:8,letterSpacing:1}}>← ENTRADA</span></div></>}
      {ehG3&&<><div style={{gridColumn:"1 / 2",gridRow:"2 / 4",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{padding:"6px 9px",borderRadius:12,border:`1px solid ${T.border}`,background:T.bgInput,color:T.textDim,fontFamily:mono,fontSize:8,letterSpacing:1}}>ENTRADA →</span></div><div style={{gridColumn:"2 / 4",gridRow:"2 / 4",display:"flex",flexDirection:"column",gap:8,alignItems:"center",justifyContent:"center",border:`1px dashed ${T.borderStrong}`,borderRadius:22,color:T.textDim,fontFamily:mono,fontSize:10,letterSpacing:2,background:`linear-gradient(145deg,${T.bgInput},${T.bgCard})`}}><span style={{width:36,height:36,display:"grid",placeItems:"center",borderRadius:12,background:T.accentBg,color:T.accent,fontSize:21,fontFamily:"sans-serif",fontWeight:300}}>＋</span><b style={{color:T.text3}}>POSTO CENTRAL</b><small style={{fontSize:7,letterSpacing:1,color:T.text4}}>EQUIPE · MONITORIZAÇÃO</small></div></>}
      {ehG4&&<><div style={{gridColumn:"4 / 6",gridRow:"2 / 4",display:"flex",flexDirection:"column",gap:8,alignItems:"center",justifyContent:"center",border:`1px dashed ${T.borderStrong}`,borderRadius:22,color:T.textDim,fontFamily:mono,fontSize:10,letterSpacing:2,background:`linear-gradient(145deg,${T.bgInput},${T.bgCard})`}}><span style={{width:36,height:36,display:"grid",placeItems:"center",borderRadius:12,background:T.accentBg,color:T.accent,fontSize:21,fontFamily:"sans-serif",fontWeight:300}}>＋</span><b style={{color:T.text3}}>ÁREA CENTRAL</b><small style={{fontSize:7,letterSpacing:1,color:T.text4}}>EQUIPE · MONITORIZAÇÃO</small></div><div style={{gridColumn:"4 / 6",gridRow:"4",display:"flex",alignItems:"flex-end",justifyContent:"center",paddingBottom:5}}><span style={{padding:"6px 12px",borderRadius:12,border:`1px solid ${T.border}`,background:T.bgInput,color:T.textDim,fontFamily:mono,fontSize:8,letterSpacing:1}}>ENTRADA ↑</span></div></>}
    </div>
  </div>;
}
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    // O HTML já declara os PNGs atuais. Não substituir por SVG inline aqui:
    // esse código antigo fazia a aba voltar ao símbolo anterior após o React montar.
    const href="/icons/uti-evolve-command-center-32.png?v=ue-command-20260906-2";
    document.querySelectorAll("link[rel~='icon']").forEach(link=>{
      link.href=href;link.type="image/png";link.sizes="32x32";
    });
  },[]);
