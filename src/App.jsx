...
const DIAGNOSTICOS_PROBLEMAS_PRESETS=[
  ...
  {nome:"Traumatismo cranioencefálico",campos:[
  ...
  {nome:"Hemorragia subaracnóidea",campos:[
  ...
  {nome:"Acidente vascular cerebral isquêmico",campos:[{key:"nihss",label:"NIHSS",tipo:"calculator"}]},
  {nome:"Tromboembolismo pulmonar",campos:[{key:"spesi",label:"sPESI",tipo:"calculator"},{key:"pesi",label:"PESI",tipo:"calculator"}]},
  ...
  {nome:"Síndrome do desconforto respiratório agudo",campos:[{key:"berlin",label:"Berlim",tipo:"select",opcoes:["Leve — P/F 201–300","Moderada — P/F 101–200","Grave — P/F ≤100"]}]},
];
const presetDiagnosticoProblema=nome=>DIAGNOSTICOS_PROBLEMAS_PRESETS.find(p=>p.nome===nome);
const detalhesDiagnosticoProblema=item=>{
  const preset=presetDiagnosticoProblema(item.nome),campos=item.campos||{};
  return [...(preset?.campos||[]).map(c=>campos[c.key]!==undefined&&campos[c.key]!==""?`${c.label}: ${campos[c.key]}`:""),...(item.custom||[]).map(c=>c.label&&c.value?`${c.label}: ${c.value}`:"")].filter(Boolean).join(" · ");
};
const textoDiagnosticoProblema=item=>{
  const detalhes=detalhesDiagnosticoProblema(item);
  return `${item.nome}${detalhes?` (${detalhes})`:""}`;
};
