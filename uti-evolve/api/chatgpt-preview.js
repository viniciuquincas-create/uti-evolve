...
import { buildPreview, newConfirmationToken, parseClinicalRequest, PREVIEW_TTL_MS, tokenDigest } from "../lib/chatgpt-clinical-flow.js";
...
export default async function handler(req,res) {
  ...
  try {
    ...
    const parsed=parseClinicalRequest(body);
    ...
    const preview=buildPreview(leitos,parsed,evolutions);
    ...
    const pending={version:2,status:"pending",previewId,createdAt:new Date(now).toISOString(),expiresAt:new Date(now+PREVIEW_TTL_MS).toISOString(),transcript:parsed.transcript,command:parsed.command,preview,confirmationDigest:tokenDigest(confirmationToken,secret)};
