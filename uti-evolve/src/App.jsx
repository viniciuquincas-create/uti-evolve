...
function SerialMeasurements({title,fieldKey,fields=[],suggestedParams=[],value,onChange,color="#38bdf8",subjective=false,calculator,patient,workflow=false,compactHistory=false,latestOnly=false}){
  ...
  const addSuggested=p=>{
    ...
    if(grupo.length){
      const keys=new Set(grupo.map(f=>f.key)),active=grupo.every(f=>customParams.some(c=>c.key===f.key));
      if(active)emit({customParams:customParams.filter(f=>!keys.has(f.key)),entries:entries.map(e=>{const values={...(e.values||{})};keys.forEach(k=>delete values[k]);return {...e,values};})});
      else emit({customParams:[...customParams,...grupo.filter(f=>!customParams.some(c=>c.key===f.key))]});
      return;
    }
