...
function CoordenacaoPanel({uti,hospital,leitos,onAbrirLeito,onVoltar,altaSheetUrl}){
  ...
  return <div style={{height:"100%",overflow:"auto",background:T.bgPage,padding:"14px clamp(12px,2vw,28px) 24px",boxSizing:"border-box"}}>
    ...
    <div style={temMapaFisico?{display:"grid",gridTemplateColumns:ehG1?"repeat(9,minmax(0,1fr))":"repeat(4,minmax(120px,170px))",gridTemplateRows:ehG1?"repeat(6,minmax(58px,1fr))":"repeat(4,minmax(88px,118px))",justifyContent:"center",alignContent:"center",gap:ehG1?8:12,width:ehG1?"100%":"min(100%,820px)",height:ehG1?"calc(100vh - 150px)":"min(calc(100vh - 165px),590px)",minHeight:ehG1?430:470,margin:ehG1?0:"14px auto 0",padding:ehG1?12:22,boxSizing:"border-box",border:`1px solid ${T.border}`,borderRadius:18,background:`radial-gradient(circle at center,${T.accentBg} 0,transparent 42%), ${T.bgCard}`,boxShadow:T.shadowCard}:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12}}>
      {ordenados.map(l=>{const n=numeroFisico(l),p=precaucaoMicrobiologica(l.culturas||[]),pos=ehG1?(G1_LAYOUT[n]||{}):ehG2?(G2_LAYOUT[String(Number(n))]||{}):ehG3?(G3_LAYOUT[String(Number(n))]||{}):{},alta=encontrarAltaDoPaciente(l.paciente,altas,leitos);return <button className="coord-bed-card" key={l.id} onClick={()=>onAbrirLeito(l.id)} style={{...pos,minHeight:58,padding:ehG1?"8px":"10px 11px",borderRadius:13,border:`2px solid ${p?.cor||T.borderStrong}`,background:p?.fundo||`linear-gradient(145deg,${T.bgInput},${T.bgCard})`,color:T.text1,cursor:"pointer",textAlign:"left",boxShadow:alta?.leitoCedido?"0 0 0 3px rgba(16,185,129,.25), 0 7px 18px rgba(15,23,42,.12)":"0 7px 18px rgba(15,23,42,.11)",overflow:"hidden",position:"relative"}} title={`${l.nome} · ${l.paciente||"Vago"}${p?` · ${p.label}`:""}${alta?` · Alta: ${alta.leitoCedido?`leito cedido ${alta.leitoCedido}`:"aguardando leito"}`:""}`}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:6}}><b style={{display:"inline-flex",alignItems:"center",justifyContent:"center",minWidth:ehG1?0:32,height:ehG1?"auto":24,padding:ehG1?0:"0 7px",borderRadius:8,background:ehG1?"transparent":T.accentBg,fontFamily:mono,fontSize:ehG1?13:14,color:p?.cor||T.accent}}>{n||l.nome}</b>{p&&<span style={{width:9,height:9,borderRadius:"50%",background:p.cor,boxShadow:`0 0 0 4px ${p.cor}18`,flex:"0 0 auto"}}/>}</div>
      ...
      {ehG2&&<><div style={{gridColumn:"2 / 4",gridRow:"2 / 4",display:"flex",alignItems:"center",justifyContent:"center",border:`1px dashed ${T.borderStrong}`,borderRadius:22,color:T.textDim,fontFamily:mono,fontSize:10,letterSpacing:2,background:T.bgInput}}>ÁREA CENTRAL</div><div style={{gridColumn:"4 / 5",gridRow:"2 / 4",display:"flex",alignItems:"center",justifyContent:"center",color:T.textDim,fontFamily:mono,fontSize:9,letterSpacing:1.5}}>← ENTRADA</div></>}
      {ehG3&&<><div style={{gridColumn:"1 / 2",gridRow:"2 / 4",display:"flex",alignItems:"center",justifyContent:"center",color:T.textDim,fontFamily:mono,fontSize:9,letterSpacing:1.5}}>ENTRADA →</div><div style={{gridColumn:"2 / 4",gridRow:"2 / 4",display:"flex",alignItems:"center",justifyContent:"center",border:`1px dashed ${T.borderStrong}`,borderRadius:22,color:T.textDim,fontFamily:mono,fontSize:10,letterSpacing:2,background:T.bgInput}}>ÁREA CENTRAL</div></>}
    </div>
  </div>;
}
